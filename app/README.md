...
- The app is the **v0b Starter Loop PWA**, feature-complete as of 2026-04-19 and deployed at https://volleydrills.nicholascorneau.workers.dev. It runs the full Home → Onboarding → Setup → Safety → Run → Check → Transition → Review → Complete loop with deterministic session assembly, setup-level focus steering, a 3-chip effort scale (Easy / Right / Hard, persisted on the 0–10 `sessionRpe` domain per `V0B-01` / `D120`), drill-grain difficulty/count capture, optional streak metric capture, a three-case honest session summary, the full Finish-Later review contract (2h cap), a flat 4-row Home priority model with a soft-block review modal, a JSON data export, block-end audio cues, and a Swap action on RunScreen. (Repeat paths from completed sessions were retired by `D158`, 2026-06-12 — the Home plan CTA and Setup's silent last-context pre-fill are the continuity paths.)
- **Routes**: `/` (Home), `/onboarding/skill-level`, `/onboarding/todays-setup`, `/setup`, `/safety`, `/run`, `/run/check`, `/run/transition`, `/review`, `/complete?id=…`, `/settings`, `/settings/skill-level`.
- **Pre-run flow**: all fresh setup and draft paths (including the Home plan CTA's silent rebuild) create or open the singleton `SessionDraft`, keep Recommended / Passing / Serving / Setting inline on Setup where applicable, then continue directly to Safety. Safety remains readiness-only.
- **Dexie schema (v7)**: `sessionPlans`, `sessionDrafts` (singleton), `executionLogs`, `sessionReviews` (with `status: 'submitted' | 'skipped' | 'draft'`, capture-window fields per `V0B-30`, optional `perDrillCaptures` per `D133`, optional streak `metricCapture` per `D134`, and optional `offeredDelta` / `verdictChoice` per `D150`/`D151`), `timerState`, `storageMeta` (key-value: onboarding progress, UX flags, last player mode). Schema in `src/db/schema.ts`. Migrations backfill `status`, `onboarding.completedAt`, and preserve the v5 drill-grain boundary; v6 adds optional streak capture and v7 adds the M002.1 next-time delta fields. The Dexie database name stays `volley-drills` to preserve any pre-rename tester data (see `D125`).
- **Session assembly + derived steering**: `buildDraft` (`src/domain/sessionBuilder.ts`, `SESSION_ASSEMBLY_ALGORITHM_VERSION` 10) consumes two derived steering inputs threaded through `BuildDraftOptions` — the `D154` stress ladder positions (`services/stressPositions.ts`) and the `D155` session-grain clock calibration (`services/calibration.ts`: upward-only overhead ratio folded from clean completes, scaling the time-profile budget before allocation). Steered callers: `startPlanSession` + Setup preview/confirm (`repeatSession`, the one verbatim unsteered caller, was retired with the Repeat affordances per `D158`). The founder JSON export (`services/export.ts`, payload `schemaVersion` 7) carries both derived reads through the same service seams (`D130` dual-read).
...
To work directly inside the app package:
...
The same checks are also exposed from the repository root as `npm run typecheck`, `npm run build`, and `npm run lint`.
