...
| 2026-06-22 | `docs/design/reviews/2026-06-22-minimalism-shibui-audit.md` | Whole-app minimalism / shibui audit ("every pixel and character fights for its life") | Five parallel read-only surface reviewers across all 13 screens + shared components against the shibui canon; ~100 findings distilled into 8 cross-cutting themes (heading-restates-button, fact-stated-thrice, decorative chrome, filler microcopy, verdict-card density, competing focal weight, surface drift, label vocabulary) + one em-dash bug; highest-leverage shortlist + low-risk quick-wins batch + founder-judgment calls; suggestions only, no code shipped |
