...
depends_on:
  ...
  - docs/archive/plans/2026-04-17-feat-phase-e-content-tooling-plan.md
...
2. **Voice is driven by `storageMeta.lastPlayerMode`; cold-state default is solo voice (revised same-day per `D128`).** The initial cut of Unit 2 kept pair voice as the cold-state default and only flipped to solo for returning solo testers, hedging for D91 evidence on "is pair-voice-first on cold first-open actively biasing solo testers?" Founder red-team surfaced the misframe pre-D91 (solo is the lead activation path per `D5`; pair-voice on screen one reads as "not for me" to the lead audience), so the cold-state default is now **solo voice** — pair voice renders only when `lastPlayerMode === 'pair'` (returning pair tester). Band descriptors stay functional-ability neutral across voices. Taxonomy enum (`D121`) is unchanged. See `D128`. Full solo/pair toggle on Skill Level (screen-zero pre-ask) stays deferred to M001-build.
...
- **Solo-voice header: should first-open pre-ask solo vs pair on a 5th option before the 4-band picker?** Tempting but adds screen time and arguably re-introduces the H9-cut Home/NewUser screen by another name. Defer: the screen-zero pre-ask stays out of v0b. **Resolved same-day by `D128`**: rather than pre-asking, flip the **cold-state default to solo voice** (consistent with `D5`, the lead activation path), and let returning pair testers see pair voice via `lastPlayerMode === 'pair'`. This closes the "pair-voice-on-cold-open biases solo testers" risk pre-D91 without adding a screen. The full solo/pair toggle on Skill Level remains an M001-build consideration with `O19` as a linked validation question.
...
| Unit 2's voice detection misfires for a returning pair user who cleared storage mid-cohort | Revised per `D128`: cold state defaults to **solo** voice, so a returning pair user who cleared storage sees solo voice on re-onboarding. The `storageMeta.lastPlayerMode` write from their NEXT pair session corrects the flip. Acceptable — the product's lead activation path is solo (`D5`), so the misfire direction is intentionally biased toward the primary audience. |
