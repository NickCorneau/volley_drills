...
- **Trigger reads.** Originally owned by the weekly Monday adversarial-memo review (per `docs/plans/2026-04-20-m001-adversarial-memo.md` Weekly trigger-review ritual) and surfaced in `docs/status/m001-validation-overhang.md` Calendar-Dated Reads. **Both surfaces are superseded as of 2026-05-27**: the validation-overhang doc is `status: superseded` (M001 closed via executive call `D147`); the Monday ritual is retired per `D148`. Live trigger-condition reads (D130 founder-use window through 2026-07-20) happen ad-hoc; the 2026-07-20 calendar date is the binding trigger. Route live posture queries to `docs/status/current-state.md`.
