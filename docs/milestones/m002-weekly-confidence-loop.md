...
- **M002.2 Stress Ladders + Technique "How" Depth — in flight, substantially shipped.** The `D154`/`D160` ladder substrate and `D159` default-path steering are in place; content has landed on top in slices (each planned + shipped 1-by-1 per `D149`): the progression-content read on Review (`D161`, capped `D162`); the run-time technique-how pair — `intent` on the block-opening get-ready beat (`D163`, relocated by the `D164`-`D167` beat-contract work) and `externalFocusCue` as the guarded live "Now" cue during the rep (`D176`, 2026-06-30, closing the technique-how pair); a roster-depth wave of seven source-backed drills (2026-06-21) plus a catalog-wide copy/tone audit; and a string of founder design-polish passes (`D168`-`D175`) refining the run-flow beat contract's density and readability. **Named remaining M002.2 work:** the Drill Check "what that rep trained" post-rep reflection (committed next slice after `D176`); a get-ready analogy depth layer; verdict-history-calibrated depth. None of these are scheduled to a new milestone — they extend M002.2 in place.
