...
- **Closed mode (2026-05-27, `D147`).** M001 is closed by founder executive call under `D130` founder-use mode. Build phase complete 2026-05-08; validation ran 2026-04-20 → 2026-05-27. The `D91` retention gate remains **deferred**, not dropped — it is preserved as the canonical bar for a future stranger launch. For live posture, route to `docs/status/current-state.md`; for closure narrative, bypass disclosure, pre-registered reversal condition, and M002 carry-forward routing, see `D147` in `docs/decisions.md`.
...
- Status: v0b Starter Loop is **feature-complete** as a D91 field-test artifact (`D119`); Phases A, B, C (C-0 → C-5), E, and F (F1 – F12) all landed through 2026-04-19. Under `D130` the build proceeds in tiers **against the full M001 product contract**, not against the v0b artifact shape. The tier structure was rescoped on 2026-04-20 after the red-team review (`docs/reviews/2026-04-20-m001-red-team.md`) surfaced scope-ballooning, SetupScreen formification, and architectural breaks in the earlier single-Tier-1 plan.
  ...
  - **Tier 1c (focus-toggle architecture, trigger fired 2026-04-28 under `D135`; pre-run route simplified by `D137`):** `sessionFocus` context field and shared focus resolver shipped, while `D137` retired the standalone `/tune-today` route. Focus selection now lives inline on Setup with **Recommended** defaulted, pre-run paths route Setup -> Safety, and `/settings/skill-level` remains the durable manual skill-level override. Trigger fired under `D135` clause 2 (partner-walkthrough ≥P1 flag) via Seb's 2026-04-27 cca2 voice memo + 2026-04-28 build-17 voice memo (both name a missing focus toggle directly).
...
- **A session-focus generation constraint** (`Pass · Serve · Set`) — surfaced inline on Setup with **Recommended** selected by default (`D137`; no standalone Tune today step). The current M001 generator keeps focus-controlled slots on one selected skill chain, but `D141` resolves `O24` by treating that as an M001 implementation constraint rather than a permanent product principle.
...
**Tier 1c — focus-toggle architecture** (trigger fired 2026-04-28 under `D135`; pre-run spine simplified by `D137`):
...
- Focus steering now lives inline on Setup with four chips: Recommended / Passing / Serving / Setting. **Recommended** is the default and focus cannot be deselected; pre-run paths route Setup -> Safety. `D137` retired `/tune-today`, `TuneTodayScreen`, the Tune today controller, `regenerateDraftFocus`, and `SessionDraft.levelRelaxed`.
