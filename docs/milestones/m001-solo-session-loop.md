...
- **Current mode (2026-04-20, `D130`): founder-use mode.** M001 is build-complete and validating against founder + partner weekly use. The `D91` retention gate is **deferred**, not dropped — it is preserved as the canonical bar for a future stranger launch and returns to the critical path at the 2026-07-20 re-eval. Build authority comes from founder conviction plus a partner walkthrough, not from D91 cohort data.
