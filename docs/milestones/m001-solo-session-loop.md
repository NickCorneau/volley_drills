...
- **Trigger-evidence status (2026-04-28).** Founder ledger gap 1 (1 of ≥8 rows) plus repeated partner-side dogfeed asks across the 2026-04-27 cca2 voice memo and the 2026-04-28 build-17 voice memo. Strict ≥8-session threshold remains binding; the partner-walkthrough OR clause requires a *scripted* walkthrough hit — dogfeed voice memos are arguably equivalent, but no policy change has been made. **Tier 1c is not unlocked.** See `docs/plans/2026-04-20-m001-tier1-implementation.md` §"Tier 1c trigger evidence accumulating" for the full reading. Skill-level mutability is tracked separately as a sibling surface with its own trigger and is **not** bundled into Tier 1c.
