...
- Status: v0b Starter Loop is **feature-complete** as a D91 field-test artifact (`D119`); Phases A, B, C (C-0 → C-5), E, and F (F1 – F12) all landed through 2026-04-19. Under `D130` the build proceeds in tiers **against the full M001 product contract**, not against the v0b artifact shape. The tier structure was rescoped on 2026-04-20 after the red-team review (`docs/reviews/2026-04-20-m001-red-team.md`) surfaced scope-ballooning, SetupScreen formification, and architectural breaks in the earlier single-Tier-1 plan.
  ...
  - **Tier 1b (content expansion on logged demand):** capped at 10 additional drill records per `docs/plans/2026-04-20-m001-adversarial-memo.md` anti-displacement cap. **Layer A candidates (5 drills, §R7 exit-3 applied 2026-04-26):** serving ladder `d31 Self Toss Target Practice`, `d33 Around the World Serving`; setting chain `d40 Footwork for Setting`, `d42 Corner to Corner Setting`, `d43 Triangle Setting`. Progression links added to serving and setting chains with dead-end fixes (pair-vs-solo branching on Rung 1 of serving; default-unlocked setting fundamentals); the chain-6-serving rung-4 leaf is `d33` until `d36` re-enters. **Deferred from Tier 1b 2026-04-26:** `d36 Jump Float Introduction` re-enters under `O7` track 2 (sports-medicine / PT review), not via Tier 1b — see `docs/decisions.md` `O7` track-2 dependency note. Layer-A cap consumption is 5/10, not 6/10. Pair opening-block (`d30 Pair Pepper Progression` + `pair_long_warmup` archetype variant) ships here **only** if partner walkthrough returns it as ≥P1.
...
- **A BAB-authored serving ladder** on `chain-6-serving` (target shape: 7 rungs, float → jump-float, with BAB drill names and VDM stage naming; ships incrementally — existing 3 rungs (`d22`, `d23`, `d24`) in Tier 1a; **Tier 1b Layer A candidates (5 drills total across serving + setting; §R7 exit-3 applied 2026-04-26):** `d31 Self Toss Target Practice`, `d33 Around the World Serving`. **Deferred from Tier 1b:** `d36 Jump Float Introduction` re-enters under `O7` track 2 (sports-medicine / PT review). Full ladder remains Tier 1b scope capped at ≤10 new drills across serving + setting; see Tier 1 plan deferred-scope table)
...
- Likely candidates (Layer A, 5 drills, §R7 exit-3 applied 2026-04-26): `d31 Self Toss Target Practice`, `d33 Around the World Serving`, `d40 Footwork for Setting`, `d42 Corner to Corner Setting`, `d43 Triangle Setting`. Order and inclusion depend on logged demand. Deferred from Tier 1b: `d36 Jump Float Introduction` (re-enters under `O7` track 2 — sports-medicine / PT review). Cap consumption is 5/10, not 6/10.
