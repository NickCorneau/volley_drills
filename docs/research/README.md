...
- Need the four candidate framings (A per-drill difficulty tag at Transition; B per-drill Good/Total at Transition; C in-session running tally on RunScreen; D hybrid tag-required + counts-optional) for the post-2026-04-26 pair rep-capture question, scored against six criteria (`D104` 50-contact math, RunScreen attention cost, post-session typing pain, authoring-budget consumption, `D125` / `D132` pair-solo voice, honesty test), plus the cross-cutting facts the framing decision must not forget (today's UI deviates from `V0B-12` drill-grain spec; the 2026-04-23 `QuickTagChips` deletion is grain-different; the full `successMetric.type` enum needs a coherent capture story; `D131` keeps a Dexie migration in scope; contract-changing framings need a `D###` proposed-decision row before the spec patch):
...
| `docs/research/2026-04-26-pair-rep-capture-options.md` | decision input for the post-2026-04-26 pair rep-capture question (Tier 1b `P2-3` trigger fired by the founder pair pass session); four framings (A per-drill difficulty tag at Transition; B per-drill Good/Total at Transition; C in-session running tally on RunScreen; D hybrid tag-required + counts-optional) scored against six criteria (`D104` 50-contact math, RunScreen attention cost, post-session typing pain, authoring-budget consumption, `D125` / `D132` voice asymmetry, honesty test); cross-cutting facts (today's UI deviates from `V0B-12` drill-grain spec; the 2026-04-23 `QuickTagChips` deletion is grain-different; the `successMetric.type` enum needs a coherent capture story; `D131` keeps a Dexie migration in scope; contract-changing framings need a `D###` proposed-decision row); does not pick a framing | does not change canon on its own; framing decision lives in `docs/decisions.md` (new `D###` row when contract-changing); spec patch lands in `docs/specs/m001-review-micro-spec.md`; Tier 1b plan lives in `docs/plans/2026-04-26-pair-rep-capture-tier1b.md` |
