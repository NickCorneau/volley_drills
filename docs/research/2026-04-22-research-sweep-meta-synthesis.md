...
| D7   | `docs/plans/2026-04-22-tier1b-serving-setting-expansion.md` `d36` row | Authoring-pointer: when `d36` `courtsideInstructions` is authored, default volume is 3×4=12 reps first-exposure, not 3×12=36. Source: `docs/research/jump-float-amateur-beach.md`. **§R7 exit-3 upgrade applied 2026-04-26**: `d36` removed from Tier 1b entirely (5 drills ship, not 6; cap consumption 5/10, not 6/10); deferred to `O7` track 2. The 3×4=12 conservative-anchor volume + prerequisite-gate framing remain inputs for the future `O7`-track-2-gated authoring plan, no longer Tier 1b constraints. | **Applied 2026-04-22-d (pointer-only); §R7 exit-3 upgrade applied 2026-04-26.** |
...
| #   | Move                                                                                                                                                                                                                                                                                                                                                                                             | Status                                                                                             |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------- |
| 1   | **§R7 exit-3 upgrade decision.** Choose between (a) keeping D7 as-applied (pointer-only — `d36` stays in Tier 1b candidate list with authoring-pointer constraining volume) or (b) the exit-3 upgrade (remove `d36` from Tier 1b candidate list entirely; re-enter `d36` when `O7` track 2 delivers).                                    | **Resolved 2026-04-26 — exit-3 chosen.** `d36` is removed from Tier 1b entirely; cap consumption drops 6/10 → 5/10; re-entry deferred to a new `O7` track 2 authoring plan (not yet authored). No replacement Layer-A drill triggered today; the freed cap slot stays open for evidence-gated future waves. Downstream propagation applied to `docs/plans/2026-04-22-tier1b-serving-setting-expansion.md`, `docs/decisions.md` `O7`, `docs/milestones/m001-solo-session-loop.md`, `docs/plans/2026-04-20-m001-tier1-implementation.md`, `docs/roadmap.md`, `docs/plans/2026-04-20-m001-adversarial-memo.md`, and `docs/research/founder-use-ledger.md`. |
...
**§R7 decision on `d36`:** **resolved 2026-04-26 — exit-3 chosen.** `d36` is removed from Tier 1b entirely and re-enters under `O7` track 2. The first-wave ship of `d31` / `d33` / `d40` / `d42` / `d43` proceeds without `d36`; the chain-6-serving rung-4 leaf is now `d33`, with the parallel-unlock to `d36` re-introduced when the future `O7`-track-2-gated authoring plan lands. Authoring-budget cap consumption is 5/10, not 6/10.
...
- ~~The founder confirms or waives the **§R7 `d36` decision in-scope for the first wave**~~ **Resolved 2026-04-26 — exit-3 chosen.** `d36` is out of Tier 1b; the first-wave ship of 5 drills (`d31` / `d33` / `d40` / `d42` / `d43`) proceeds without `d36`. Re-entry under `O7` track 2 has its own authoring plan (not yet authored).
