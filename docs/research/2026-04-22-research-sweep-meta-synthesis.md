...
- **D7** Tier 1b `d36` authoring-pointer — originally applied to the `d36` row in `docs/plans/2026-04-22-tier1b-serving-setting-expansion.md` with pointer-only language (default 3×4=12 first-exposure; 3×12=36 preserved as later-wave steady-state target). **§R7 exit-3 upgrade applied 2026-04-26**: `d36` removed from Tier 1b entirely; deferred to `O7` track 2 (sports-medicine / PT review) — see §R7 below for the exit-options analysis and `docs/plans/2026-04-22-tier1b-serving-setting-expansion.md` §"§R7 exit-3 applied 2026-04-26" for the downstream propagation. The 3×4=12 conservative-anchor volume default and the prerequisite-gate framing remain inputs for the future `O7`-track-2-gated authoring plan; they are no longer Tier 1b authoring constraints.

All canon closeouts from this sweep are now applied; the §R7 exit-3 upgrade discharged on 2026-04-26 closes the last open founder decision from D7. The meta-synthesis is coherent with `docs/decisions.md`, `docs/vision.md`, and `docs/roadmap.md`. Downstream work can resume.
...
| D7   | `docs/plans/2026-04-22-tier1b-serving-setting-expansion.md` `d36` row | Authoring-pointer: when `d36` `courtsideInstructions` is authored, default volume is 3×4=12 reps first-exposure, not 3×12=36. Source: `docs/research/jump-float-amateur-beach.md`. **§R7 exit-3 upgrade applied 2026-04-26**: `d36` removed from Tier 1b entirely; deferred to `O7` track 2. **2026-04-27 source red-team correction**: `d43` also deferred to `D101`, so current Layer A ships 4 drills, not 6; cap consumption 4/10, not 6/10. The 3×4=12 conservative-anchor volume + prerequisite-gate framing remain inputs for the future `O7`-track-2-gated authoring plan, no longer Tier 1b constraints. | **Applied 2026-04-22-d (pointer-only); §R7 exit-3 upgrade applied 2026-04-26; `d43` correction applied 2026-04-27.** |
...
- **M001 scope and Tier 1a / 1b / 1c / 2 ordering** (`docs/milestones/m001-solo-session-loop.md`). Unchanged by this sweep's deltas. The research does not re-open the tier breakdown, does not unblock Tier 1b, does not elevate `d36` to Tier 1a, does not add a baseline-test surface to M001. (Note: D132's scope explicitly preserves M001 tactical shape; the "Solo Session Loop" milestone name is tactical, not strategic.) **2026-04-26 update**: `d36` is now deferred from Tier 1b entirely per §R7 exit-3 — it re-enters under `O7` track 2 (sports-medicine / PT review), not via Tier 1b promotion or Tier 2 advancement.
...
**Exit (3) is the honest exit.** D7 as applied 2026-04-22-d (pointer-only) implements exit (2) implicitly — it constrains `d36` authoring to 12 reps but does not enforce gate architecture. **The §R7 exit-3 upgrade was applied 2026-04-26**: `d36` is removed from Tier 1b entirely; it does not consume the authoring-budget cap; it re-enters under `O7` track 2 (sports-medicine / PT review) via a new authoring plan that wires the prerequisite gate, the warm-up dependency, and the conservative-anchor first-exposure volume defaults. The 2026-04-21 founder-named serving-content gap (ledger gaps 2 + 3) is fully addressed by `d31` and `d33` without `d36`; replacement with a different Layer-A-fitting serving drill is **not** triggered today (the four remaining cap slots stay open for evidence-gated future waves). Recorded here so the exit-2 → exit-3 trajectory and rationale are preserved for the future re-entry plan.
...
| #   | Move                                                                                                                                                                                                                                                                                                                                                                                             | Status                                                                                             |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------- |
| 1   | **§R7 exit-3 upgrade decision.** Choose between (a) keeping D7 as-applied (pointer-only — `d36` stays in Tier 1b candidate list with authoring-pointer constraining volume) or (b) the exit-3 upgrade (remove `d36` from Tier 1b candidate list entirely; re-enter `d36` when `O7` track 2 delivers).                                    | **Resolved 2026-04-26 — exit-3 chosen.** `d36` is removed from Tier 1b entirely; 2026-04-27 source red-team correction also defers `d43` to `D101`, so current Layer A cap consumption is 4/10; re-entry for `d36` is deferred to a new `O7` track 2 authoring plan (not yet authored), and re-entry for `d43` waits for 3+ player support. No replacement Layer-A drill triggered today; the freed cap slots stay open for evidence-gated future waves. Downstream propagation applied to `docs/plans/2026-04-22-tier1b-serving-setting-expansion.md`, `docs/decisions.md` `O7`, `docs/milestones/m001-solo-session-loop.md`, `docs/plans/2026-04-20-m001-tier1-implementation.md`, `docs/roadmap.md`, `docs/plans/2026-04-20-m001-adversarial-memo.md`, and `docs/research/founder-use-ledger.md`. |
| 2   | **Partner-walkthrough closeout polish pass (Tier 1a Wave 2).** Ship a bounded bundle from the `2026-04-22-all-passes-reconciled.md` open-list.                                                                                                                                                                                                                                                   | **Landed 2026-04-24** as `docs/archive/plans/2026-04-23-walkthrough-closeout-polish.md` (four items: RPE 3-anchor collapse + Quick-tags deletion + divider + Done/Finish-later equal weight + non-count Good-passes hide + 2-hour Review-window copy removal + Safari-caveat compression). Items 1 and 4 of the plan discovered as already-shipped in the 2026-04-22 polish pass. |
...
3. **New finding surfaced, not on any prior tracked list: timer-end screen-off anxiety / audio-suspend-on-lock.** iOS Safari PWA suspends audio when the screen locks; the block-end beep cannot be relied on. The 2026-04-22 preroll "keep phone unlocked" hint was gated first-time-only in the polish ship, which dismissed it for the two users most likely to be watching for the problem. The structural mitigation is the **Screen Wake Lock API** (iOS Safari 16.4+) requested from a real user gesture where possible, not copy alone. Captured as a new item in `docs/research/partner-walkthrough-results/2026-04-22-all-passes-reconciled.md` §"Genuinely-open Tier 1b bundle" and **landed 2026-04-24** via gesture-bound wake-lock/audio priming + `useWakeLock` lifecycle hardening + tests; manual device lock remains a platform boundary.
...
**Sequencing the four exit-3-compatible Tier 1b drill records:**
...
3. **`d40 Footwork for Setting`**, **`d42 Corner to Corner Setting`** (setting chain). Addresses Condition 1's set-floor (currently 0 of 3) — when either lands, founder + Seb can reach set-focused sessions. `d43 Triangle Setting` is deferred to `D101` 3+ player support because the BAB source form is 3-player.

**§R7 decision on `d36`:** **resolved 2026-04-26 — exit-3 chosen.** `d36` is removed from Tier 1b entirely and re-enters under `O7` track 2. **2026-04-27 source red-team correction:** `d43` is also deferred to `D101` 3+ player support. The first-wave ship of `d31` / `d33` / `d40` / `d42` proceeds without `d36` or `d43`; the chain-6-serving current leaf is `d33`, with any future `d36` link re-introduced only by the future `O7`-track-2-gated authoring plan. Authoring-budget cap consumption is 4/10, not 6/10.
...
- ~~The founder confirms or waives the **§R7 `d36` decision in-scope for the first wave**~~ **Resolved 2026-04-26 — exit-3 chosen.** `d36` is out of Tier 1b; the first-wave ship of 4 drills (`d31` / `d33` / `d40` / `d42`) proceeds without `d36`. **2026-04-27 source red-team correction:** `d43` also waits for `D101` 3+ player support. Re-entry under `O7` track 2 has its own authoring plan (not yet authored).
