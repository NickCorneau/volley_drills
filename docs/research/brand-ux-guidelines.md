...
| `/` | resume, review-pending, plan launch, local trust cues | Inspect trust/resume roles; apply only documented hard violations |
...
- **Setup is recommendation-first (`D158`, 2026-06-12 shibui comp review, `setup-01` frame).** A focal resolved line tops the body in `text-lg font-semibold leading-snug` (two-line wrap accepted). The focus segment reflects `D159` resolved provenance: with the **Recommended** pill active and the derived plan's focus resolved, the line names it — `{archetype} · {assembled min} min · {Focus} (recommended)` (e.g. `Solo + Net · 25 min · Passing (recommended)`); a user-picked focus reads `{Focus} focus` (e.g. `Pair + Net · 38 min · Setting focus`); only the unstamped fallback (focus resolution unavailable — plan-input load failed or the stamped build returned null) reads the generic `Recommended focus`. The minute segment always reads the **assembled preview total** — the same number the footer Callout reports — never the named Time-chip profile, so the screen's two duration statements cannot disagree (duration-honesty R4). While Setup is incomplete the slot carries the existing `Choose … to build.` hint in quiet secondary voice (`text-sm text-text-secondary`); the footer hint by the disabled Build button stays too (both shown in the approved frame 06).
...
- Single `Now` cue surface (non-segmented) OR `SegmentList` (segmented). The remaining coaching cues and the full `courtsideInstructions` read are both one tap away behind a single `Drill details` overlay (`D165` no-pause recovery, merged `D169` 2026-06-29; the read section is omitted for segmented drills where the `SegmentList` already carries the read, so it is never duplicated). This supersedes the 2026-05-25 H2 inline `Show more cues and instructions` `<details>` treatment, which the run-flow beat contract retired (Stage 1, `D164`): the live cockpit no longer renders the prose read inline — its single full-weight home is the Run get-ready READ-DO beat (`D167`).
