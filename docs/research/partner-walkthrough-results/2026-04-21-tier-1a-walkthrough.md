...
Dispositions recorded 2026-04-21 during the founder pre-close review pass. Each row is one of `Accept (landed)` / `Accept (pre-close)` / `Tier 1b` / `Tier 1c` / `Tier 2` / `Script-only` / `Wontfix-info`. Pre-close items shipped in the same 2026-04-21 batch that produced this dispositions table; Tier 1b / Tier 2 items carry explicit behavioural triggers naming the founder-use-ledger or walkthrough evidence that advances them out of deferred status. Wontfix-info = kept as positive evidence, no action.

| # | Finding | Severity | Disposition | Rationale |
| - | ------- | -------- | -------------------- | --------- |
| P1-1 | Skill-level screen not readable as a skill-level question | P1 | **Accept (pre-close)** | Copy fix (headline-as-question + "Your rough current level..." subtitle) was landed pre-walkthrough; the remaining gap was visual hierarchy of "Not sure yet." Fix: promoted from `variant="link"` text-button to a fifth focal-surface option card inside the same `<ul>` as the four bands, with a recommend-first descriptor ("We'll size a light starter - you can change this after."). Taxonomy enum unchanged. See `SkillLevelScreen.tsx` + `SkillLevelScreen.test.tsx`. |
| P1-2 | Home / post-session navigation confusion | P1 | **Accept (landed + pre-close)** | CompleteScreen primary CTA already renamed "Done" → "Back to home" in the 2026-04-21 polish pass. Pre-close: renamed every remaining "Back to start" recovery link across `RunScreen`, `TransitionScreen`, `ReviewScreen`, `CompleteScreen.missing`, `SessionGuard`, `ErrorBoundary` so the destination is consistent across error/empty states. |
| P1-3 | Task 2 swap-to-serving impossible on solo_open | P1 | **Accept (script-side) · Wontfix (app-side)** | Script fixed upstream in `docs/research/partner-walkthrough-script.md` (Task 2 no longer implies serving on `solo_open`). App-side: no change. Seb's Pass 1 Q7 answer on the Swap gesture itself was **"Right gesture, felt intuitive."** The 13-swap frustration was script-induced (Task 2 asked him to reach serving content `solo_open` doesn't carry), not a Swap-UX failure. An earlier pre-close prototype added a next-alternate preview label on the Swap button; rolled back on founder review as an over-fit onto a script signal. App-side Swap changes stay parked until the founder-use ledger surfaces an actual Swap-UX signal (swap-regret, "didn't know what it would do," etc.). The draft-screen workout-preview / list-picker remains the scoped Tier 2 surface (v0b plan §4) for "I want to see the whole session before running it." |
| P1-4 | Beach Prep vocabulary ("drop-step") | P1 | **Accept (landed)** | d28 Beach Prep Three courtside copy rewritten to "pivot-back starts at game pace" per courtside-copy rule §2 flagged-vocabulary table. Rule will catch regressions. |
| P1-5 | "Set window" / "pass window" vocabulary | P1 | **Accept (landed)** | Inline gloss pattern "set window (where the setter would stand, ~3 m off the net)" applied on first occurrence per drill across the catalog (8 sites). Courtside-copy rule §2 pins the invariant. Tap-to-expand glossary is Tier 2 (same bucket as See-Why modal). |
| P1-6 | 6-Legged Monster coach cue ambiguity | P1 | **Accept (landed)** | d17 coach cue now reads "Drop near shoulder, lift far shoulder on wide passes." per courtside-copy rule §2 mapping (inside/outside → near/far from the reader's perspective). |
| P1-7 | 6-Legged Monster toss matrix too mathematical | P1 | **Accept (landed)** | d17 courtside copy now enumerates the six spots explicitly ("front-left, side-left, behind-left, front-right, side-right, behind-right"). Codified as courtside-copy rule §3 ("No combinatoric descriptions"). |
| P1-8 | Cool-down stretch abstraction | P1 | **Accept (landed) + Tier 1b (tap-to-expand)** | d26 Lower-body Stretch Micro-sequence now enumerates each stretch with an inline execution cue (1–6 with form cues). Tap-to-expand per-stretch demo component remains Tier 1b; trigger is founder logging ≥1 session where more form detail was wanted mid-wrap, OR partner walkthrough surfacing the need. |
| P1-9 | Swap preview / 3-s undo | P1 (weak) | **Tier 1b, on trigger** | Quote fragmentary; avoid building against ambiguous signal. Preferred shape when triggered is undo-toast (outdoor-UI brief: "Prefer undo over confirm."), not pre-commit preview. Trigger: founder logs ≥1 swap-regret event ("swapped to X, wanted Y back") in `founder-use-ledger.md`. Pre-close landed item F (Swap button preview label) may make this unnecessary by fixing the underlying "what will I get" uncertainty. |
| P1-10 | Duration framing (warm-up + cool-down inside 15 min) | P2 | **Accept (landed)** | One-line clarifier "Includes warm-up and cool-down." already shipped under the time-chip picker in the 2026-04-21 polish pass. |
| P1-11 | "Chosen because:" merged with coach cues | P2 | **Accept (pre-close)** | Rationale relocated **above** `courtsideInstructions` as a subtitle under the drill name (was rendered below the coach-cue toggle, which caused the merge). Typography bumped from `text-xs` (12 px, floor violation per outdoor-UI brief) to `text-base` (16 px floor). `RunScreen.rationale-placement.test.tsx` pins both invariants. |
| P1-12 | RPE scale labels visibility | P2 | **Accept (landed)** | `RpeSelector` anchor rail removed; per-chip aria-label + live `SELECTED_HINT` now carry the Borg meaning without consuming persistent vertical space. |
| P2-1 | Courtside text not readable at arm's length | P1 | **Accept (pre-close, partial) + Tier 2 (distance-mode typography, sand-gated)** | Pre-close: typography floor-bump on `RunScreen` (rationale `text-xs` → `text-base`; `courtsideInstructions` `text-sm` → `text-base`; `coachingCue` `text-sm` → `text-base`). This is enforcement of the outdoor-UI brief's existing 16 px body floor — founder-review thought 2 confirmed the readability gap was already present indoors (not sand-specific). Bench-distance-mode typography (timer digits 72–88 px) remains validate-later per the brief; trigger: ≥2 founder sand sessions with readability flagged in `founder-use-ledger.md`. |
| P2-2 | 30-s pacing beeps + pair swap-roles cue | P1 | **Accept (pre-close — warm-up + cool-down pacing) + Tier 1b (pair role-swap cue)** | Pre-close: shipped sub-block pacing tick infra — new `playSubBlockTick()` (1400 Hz) in `lib/audio.ts`, `variant.subBlockIntervalSeconds` plumbed through DrillVariant → DraftBlock → SessionPlanBlock → RunScreen, wired for d28 Beach Prep Three (45 s) and d26 Stretch Micro-sequence (30 s — matches Seb's explicit "beep every 30 seconds during the cool down" ask). Also shipped block-end 3-sec countdown ticks (tick-tick-tick-BEEP mirroring the preroll entrance ramp). Pair role-swap cue deferred to Tier 1b — requires drill-metadata work (`variant.roleSwapAtMinute?`), authored when founder logs ≥2 pair sessions with unclear role transitions. |
| P2-3 | Pair rep capture friction | P2 | **Accept (pre-close — notCaptured default) + Tier 1b (drill-metadata-driven capture UI)** | Pre-close sub-fix: ReviewScreen now defaults `notCaptured` to selected when the main_skill drill's `successMetric.type` is not count-based (`pass-rate-good` / `reps-successful`). Tester no longer feels obligated to invent numbers on `streak` / `points-to-target` / `pass-grade-avg` / `composite` / `completion` drills; can toggle off to record real numbers. Full drill-metadata-driven capture UI (hide Good/Total entirely for non-count drills; branch PassMetricInput on capture style) is Tier 1b — trigger: founder logs ≥2 sessions where the default feels wrong, OR ≥1 pass-rate-good session where the guess was explicitly noted as fake. |
| P2-4 | Script Pass 2 Q4 wording | P2 | **Script-only (landed)** | Fixed in `docs/research/partner-walkthrough-script.md` in the 2026-04-21 polish pass. |
| P2-5 | 6-Legged Monster fun / observed improvement | info | **Wontfix-info — keep as positive evidence** | Recorded in 2026-04-27 weekly log entry: "d17 validated as a good-shape drill once vocabulary is readable — the copy/cue rewrites (P1-6, P1-7) do not need to reshape the drill structure itself." |
