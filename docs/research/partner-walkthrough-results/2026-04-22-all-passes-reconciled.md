...
| 9. Block-end countdown cue on Run                                   | **Fully landed.** Seb P2-2 pre-close landed: audio block-end tick-tick-tick-BEEP via `playSubBlockTick()` (1400 Hz) + entrance-ramp 3-sec countdown. **Visual countdown cue** (thicker progress bar + "0:47 left" chip) from design review A4 landed 2026-04-26 in `BlockTimer.tsx` — bar grew `h-2` → `h-3`, new `"X:XX left"` chip on the bar's trailing edge, big-digit final-3-sec accent flip preserved as the single visual countdown tick (chip stays in `text-text-secondary`). Coverage: `BlockTimer.progress-chip.test.tsx`. | Seb P2-2                      |
...
3. **Finish the Review cut** (RPE 11 → 3 anchors; delete Quick tags; divider-line treatment; delete 2-hour countdown; `Done` primary + lower-emphasis `Finish later`; fully hide Good-passes card on non-count drills, not just default-notCaptured). **[Landed 2026-04-24; footer hierarchy revised same-day]** per `docs/archive/plans/2026-04-23-walkthrough-closeout-polish.md` items 2 + 3. RPE chips are now `Easy / Right / Hard` mapping to canonical sessionRpe 3 / 5 / 7 via `app/src/components/rpeSelectorUtils.ts` (non-canonical historical values snap to nearest chip for display, not mutated on disk); `QuickTagChips` component deleted; hairline divider between RPE and Good-passes; 2-hour `This session stops counting in about N hr M min` subtitle removed; Submit → Done; 2026-04-24 founder design feedback reversed the equal-primary Done / Finish later experiment, so Finish later is again a link-style escape hatch beneath the primary Done button; Good-passes gated on `inferMainSkillMetricType` being count-based. [W + S + D, building on Seb P1-12 + P2-3]
4. **Delete 2-hour Review-window copy**; compress Complete Safari-eviction footnote to Settings; keep the `✓ Saved on this device` line. **[Landed 2026-04-24]** per `docs/archive/plans/2026-04-23-walkthrough-closeout-polish.md` item 3. Posture-sensitive secondary moved into a new Settings `About local storage` sub-section driven by the same `getStorageCopy(posture)` source of truth; Complete carries a small `Why is this?` link. `D118` three-state durability posture unchanged. [W + S + D]
...
8. **Visual block-end countdown cue** on Run — thicker progress bar + "0:47 left" chip. **[Landed 2026-04-26]** in `app/src/components/BlockTimer.tsx`: progress bar grew from `h-2` (~8 px, accent-only, read as decoration at 2 m courtside) to `h-3` (~12 px), with a new `"X:XX left"` chip on the bar's trailing edge using `formatTime(remainingSeconds)` so chip + bar + big digits all agree on remaining time. Big-digit final-3-sec accent flip is preserved untouched (the chip stays in `text-text-secondary` and does not flip — the digits remain the single visual countdown tick). Coverage in `app/src/components/__tests__/BlockTimer.progress-chip.test.tsx` (bar height, chip text, paused state, countdown-window non-flip, fill-ratio agreement). Complements the already-landed audio tick (Seb P2-2) and the 2026-04-24 Wake Lock + audio-prime ship (item #17): bar/chip provide the at-a-glance visual cue that survives muted phone, sand wind, and ambient noise where the audio tick is reduced or absent. [D A4]
...
- ✅ Review cut completed per merged proposal (item 3). [2026-04-24 closeout polish — RPE 3-chip, Quick tags deleted, divider, Done primary + lower-emphasis Finish later link after same-day founder design reversal, Good-passes gated on count metric]
...
- ✅ Visual block-end countdown cue on Run (item 8). [2026-04-26 — `h-2` → `h-3` progress bar + `"X:XX left"` chip in `BlockTimer.tsx`; chip stays in `text-text-secondary` so the existing big-digit final-3-sec accent flip remains the single countdown tick. Coverage: `BlockTimer.progress-chip.test.tsx`.]
...
- ✅ Screen Wake Lock + gesture-bound audio prime on active session (item 17). [2026-04-24 hook hardening + Safety Continue wake-lock/audio prime — wake lock is requested from the user gesture, re-acquires on visibility return, releases on explicit pause / unmount, graceful fallback when unsupported; audio context created/resumed from a real tap before Run preroll; manual device lock remains a documented platform boundary]
