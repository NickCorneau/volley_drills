...
- **Parallel / infrastructure items discovered during founder-use** (surfaced 2026-04-24, not part of the Tier 1b drill cap): Screen Wake Lock API on active sessions to prevent iOS Safari PWA audio-suspend-on-lock. **Landed 2026-04-24** via `useWakeLock` lifecycle hardening (request while Run timer is active, release on pause / transition / unmount, re-acquire on visible return after browser release, graceful fallback when unsupported). Captured as reconciled-list item #17 in `docs/research/partner-walkthrough-results/2026-04-22-all-passes-reconciled.md`.
...
- Screen-lock audio-suspend failure mode (iOS Safari PWA): iOS Safari suspends audio when the screen locks, which means block-end beeps and sub-block pacing ticks cannot be relied on if a user's screen turns off mid-session. This surfaced as a concrete user anxiety in the 2026-04-21 founder joint-session ledger (gap 4). The 2026-04-22 preroll "keep phone unlocked" hint addresses the copy-level version of this and is gated first-time-only (dismissed users no longer see it). **Mitigation landed 2026-04-24**: `useWakeLock` requests Screen Wake Lock while Run timer is active, releases on pause / transition / unmount, re-acquires on visible return after browser release, and no-ops on unsupported browsers. Captured as reconciled-list item #17; infrastructure, not content; did not consume the Tier 1b drill cap. Residual risk: older Safari without Wake Lock support still relies on the copy hint and user behavior; a future visual block-end countdown cue (#8) still improves trust but does not replace Wake Lock.
