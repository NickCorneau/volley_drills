...
- Research-velocity substitution: research volume can feel like progress while no behavioral-evidence channel advances. Behavioral evidence for `D130` and the 2026-07-20 re-eval includes (a) founder personal-use sessions logged in `docs/research/founder-use-ledger.md`, (b) partner (Seb) usage and unprompted-open cadence, (c) partner-walkthrough observations under `docs/research/partner-walkthrough-results/`, and (d) Condition 3 signals. The 2026-04-22 nine-vendor research sweep (`docs/research/2026-04-22-research-sweep-meta-synthesis.md`) landed while the partner-usage and walkthrough channels were actively producing signal (Seb T+1-day unprompted open on 2026-04-22 produced the `D130` Condition 3 provisional pass) but the founder-ledger channel was at zero rows; the 2026-04-22-c founder correction clarifies that personal-ledger logging is not a canon-edit gate and Seb's usage is first-class evidence. The real failure mode is research outpacing **all** behavioral-evidence channels, not just the ledger. Mitigate by the weekly Monday adversarial-memo review reading research-note count alongside the full set of behavioral-evidence channels (flag if research notes added ≥ 3 in a week and every channel at (a)–(d) is silent), and by applying research findings to canon only through the `[apply now]` vs `[needs review]` discipline flagging in the meta-synthesis note rather than in a single activate-everything pass. See `docs/research/founder-use-ledger.md` §"This ledger is one behavioral-evidence channel, not the only one" and `docs/research/2026-04-22-research-sweep-meta-synthesis.md` §R3 (revised).
