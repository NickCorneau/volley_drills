...
- **Parallel / infrastructure items discovered during founder-use** (surfaced 2026-04-24, not part of the Tier 1b drill cap): Screen Wake Lock API + gesture-bound audio priming on active sessions to prevent iOS Safari PWA audio-suspend-on-lock and autoplay blocking. **Landed 2026-04-24** via `useWakeLock` lifecycle hardening (request while Run timer is active, release on pause / transition / unmount, re-acquire on visible return after browser release, graceful fallback when unsupported) plus `primeAudioForGesture()` from the Safety Continue tap so the first preroll tick is not the first attempted `AudioContext` creation on iOS. Captured as reconciled-list item #17 in `docs/research/partner-walkthrough-results/2026-04-22-all-passes-reconciled.md`.
