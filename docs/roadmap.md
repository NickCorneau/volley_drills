...
- **Tier 1c (focus-toggle architecture, evidence-gated)**: `sessionFocus` field on context, dynamic `slot.skillTags` override in `sessionBuilder.ts::pickForSlot` AND `findSwapAlternatives`, Swap-Focus button on the **draft screen** (not SetupScreen). Behavioral signals accumulating across two channels through 2026-04-28: (i) founder ledger gap 1 captured 2026-04-24 ("we dont get to pick focuses as far as i can tell") — 1 row, threshold is ≥8; (ii) partner-side dogfeed asks repeated across the 2026-04-27 cca2 voice memo and the 2026-04-28 build-17 voice memo — Seb explicitly named focus selection ("today I want to do 30 minutes of serving — let me select that") and called the landing-page / difficulty-or-skill-level entry surface his "consistent point." Strict reading: ≥8-session ledger threshold is at 1; the partner-walkthrough OR clause needs a *scripted* walkthrough hit (dogfeed voice memos are arguably equivalent, but that policy call is not made). **Not yet unlocked.** Skill-level mutability tracks as a sibling surface with its own trigger (see `docs/plans/2026-04-20-m001-tier1-implementation.md` §"Skill-level mutability — separate surface, separate trigger"); it is not bundled into Tier 1c.
