...
Current planning default for the first implementation-ready milestone: **tactically** solo-first flow with lightweight pair fallback and a passing-fundamentals-for-serve-receive first track. Solo-first here is an activation tactic (reduces the "can I try this today" barrier) inside the pair-first strategic frame established in `docs/vision.md` — M001's "Solo Session Loop" name describes the thin-slice activation shape, not the product's end-state; under pair-first, every solo session in M001 is framed as training in service of the athlete's pair performance. The broader MVP envelope may grow into a shallow next-N session queue and a minimal weekly receipt, but coach-led overlays remain downstream of the **M002 Weekly Training Home series** and the coach gate in Roadmap intent. Inside Phase 1, **M001** proves the runner loop and the **M002 Weekly Training Home series** (renamed from "Weekly Confidence Loop" per `D149`) makes the product feel worth returning to before any coach-connected build begins; its progression-spine core (M002.1–M002.3) is the Phase-1 "main-tool pull" target, with goals/content/roster tracks leaning into Phase 1.5 territory.

**Under `D130` (2026-04-20), Phase 1's M001 was built in tiers for founder + partner use.** Tier 1a, Tier 1b Layer A, and Tier 1c shipped; Tier 2 polish gate unlocked 2026-05-23 (Condition 3 PASS) but no Tier 2 surface shipped before closure. **M001 closed 2026-05-27 by founder executive call (`D147` in `docs/decisions.md`)** ahead of the pre-registered 2026-07-20 D130 re-eval — bypass disclosure auditable in the D147 row. The 2026-07-20 re-eval still fires with reframed scope (D130 window close + friends-of-friends cohort question on M002 evidence; does NOT re-read M001 closure). See `docs/status/current-state.md` Recent Shipped History for the closure narrative. The tier structure was rescoped on 2026-04-20-d after the red-team review (`docs/reviews/2026-04-20-m001-red-team.md`) surfaced scope-ballooning, SetupScreen formification, and architectural breaks in the earlier single-Tier-1 plan. Current validation gates live in `docs/status/m001-validation-overhang.md`.
...
- **Tier 1a (minimum shippable content + safety base) — SHIPPED.** Warm-up authoring bug fix (one drill `d28 Beach Prep Three`); setting minimum probe (3 drills — `d38`, `d39`, `d41` — `chain-7-setting` with no progression links in Tier 1a); vocabulary sweep with inline definitions; "Chosen because:" rationale on RunScreen; last-3-sessions row on Home. Shipped through 2026-04-21. **No new SetupScreen toggles.** **No focus-routing architecture.** **No pair opening-block.** See `docs/plans/2026-04-20-m001-tier1-implementation.md`.
  - **Tier 1a polish passes shipped.** Partner-walkthrough polish (2026-04-22, 6 items: safety recency chip relabeling, neutral disabled CTA token, PainOverrideCard microcopy, first-session verdict on Complete, preroll hint gated first-time-only, Shorten-block Transition styling). Walkthrough closeout polish (2026-04-23, 4 items — 2 shipped fresh + 2 discovered as already-landed: Review merged-proposal remainder [RPE 3-anchor + Quick-tags deletion + divider + Done/Finish-later equal weight + non-count Good-passes hide], 2-hour Review-window copy removed + Safari-caveat compressed to Settings). Pre-D91 editorial polish (2026-04-26, 6 original items + 3 same-day post-ship amendments: `F11` review-pending eyebrow voice, `F12` recent-workouts `'partial'` → `Mixed`, `F7` Good-passes `0/0` placeholder, `F9` `Completed session N` ordinal removed, `F10` solo `Today's verdict` eyebrow dropped (kept on pair), `F14` `Build {sha} · {date}` row in Settings for D91 field-test debugging; amendments: redundant `BlockTimer` time-chip removed once `text-[56px]` digits arrived, `Difficulty` aggregator + recap row, and Item 9 — splitting per-drill capture onto its own `/run/check` screen so reflective and rehearsal beats no longer share a viewport). See `docs/plans/2026-04-22-partner-walkthrough-polish.md`, `docs/archive/plans/2026-04-23-walkthrough-closeout-polish.md`, and `docs/archive/plans/2026-04-26-pre-d91-editorial-polish.md`.
- **Tier 1b (content expansion on logged demand) — drill-authoring Layer A and pair rep-capture shipped 2026-04-27.** Founder-session trigger for the drill-authoring track formally met on 2026-04-24 via the 2026-04-21 joint-session ledger backfill + founder-named serving-content gap (see `docs/research/founder-use-ledger.md` §"Content gaps surfaced on the 2026-04-21 sessions"). Capped at ≤10 new drills total per `docs/plans/2026-04-20-m001-adversarial-memo.md` anti-displacement cap. **Layer A authored 4 drills:** `d31`, `d33` from serving; `d40`, `d42` from setting — authoring spec in `docs/plans/2026-04-22-tier1b-serving-setting-expansion.md`. Serving uses safe same-chain links (`d22` / `d31` → `d33`, plus lateral `d22` → `d31`); setting keeps fundamentals default-unlocked and links `d41` → `d42` only. **Deferred from current Layer A:** `d36 Jump Float Introduction` (founder decision: §R7 exit-3 chosen) re-enters under `O7` track 2 (sports-medicine / PT review); `d43 Triangle Setting` re-enters under `D101` 3+ player support because the BAB source geometry is 3-player. Cap consumption is 4/10, not 6/10; the freed slots stay open for evidence-gated future waves and are not auto-backfilled. Pair opening-block (`d30` + `pair_long_warmup` archetype variant) ships here only if walkthrough returns the need. **Pair rep-capture track (separate Tier 1b slot, no drill-cap consumption) — SHIPPED 2026-04-27.** `P2-3` trigger fired by the 2026-04-26 founder pair pass session (`d11` + `d03` + `d10`, `pass-rate-good` drills with explicit "fake count" flag) per `docs/research/founder-use-ledger.md`. `D133` (Framing D from `docs/research/2026-04-26-pair-rep-capture-options.md`) ratified and implemented same-week: per-drill **Difficulty tag** (required) + per-drill **Good/Total** (optional) on a new `/run/check` route (`DrillCheckScreen.tsx`) sitting between Run and Transition; Transition refocused on Up Next; session-level `Good passes / Total attempts` card removed from Review for count drills (`successMetric.type` ∈ `pass-rate-good` / `reps-successful`); Complete recap aggregates from drill-grain when present and surfaces a quiet `Difficulty` row; **Dexie schema bumped to v5** with `perDrillCaptures: PerDrillCapture[]` (forward-only). The current live schema is v7 (v6 added optional streak `metricCapture`; v7 added the M002.1 `offeredDelta` / `verdictChoice` review fields per `D150`/`D151`). See `docs/plans/2026-04-26-pair-rep-capture-tier1b.md` and the post-ship Item 9 amendment in `docs/archive/plans/2026-04-26-pre-d91-editorial-polish.md`.
- **Tier 1c (focus-toggle architecture, trigger fired 2026-04-28 under `D135`; Tune today shipped 2026-04-30)**: `sessionFocus` field on context, shared focus resolver, in-transaction draft regeneration, and mandatory `/tune-today` route after draft creation/opening and before Safety. Trigger fired under `D135` clause 2 (partner-walkthrough ≥P1 flag) via Seb's 2026-04-27 cca2 voice memo + 2026-04-28 build-17 voice memo — Seb explicitly named focus selection ("today I want to do 30 minutes of serving — let me select that") and called the landing-page / difficulty-or-skill-level entry surface his "consistent point" across both sessions. Founder-side ledger evidence (gap 1 from 2026-04-21, "we dont get to pick focuses as far as i can tell") corroborates as first-class founder content-gap evidence under `D135`, but the independent ≥8-session founder-only clause is not at threshold (1 / 8) — clause 2 is the firing path. Skill-level mutability tracks as a sibling surface with its own trigger (also fired 2026-04-28 under `D135` via the same two memos naming skill-level inflexibility directly; see `docs/plans/2026-04-20-m001-tier1-implementation.md` §"Skill-level mutability — separate surface, separate trigger"); it is not bundled into Tune today and remains paused pending dogfood.
- **Tier 2 (deferred-surfaces unblock)**: full "See why" modal, richer summary copy, full session history screen, recommendation-first first-run polish to `D123`. Gated on Tier 1a acceptance + adversarial-memo Condition 3 pass (partner unprompted open within 30 days of Tier 1a walkthrough) + two weeks of founder weekly use. Condition 3 is at **provisional pass, strengthened** as of 2026-05-10 (Seb T+1 unprompted + T+2 instrumented opens, plus the 2026-05-10 partner session export as usage evidence but not unprompted-open proof); final read-out 2026-05-21. Two weeks of founder weekly use is accumulating through `docs/research/founder-use-ledger.md` (rows logged through 2026-05-10; running total 7). The latest standard `D130` Condition 1 read is fail-trending (solo 1/7, set 0/3) and is a 2026-07-20 re-read concern, not a reason to pull Tier 2 forward.
...
- **M002 (Weekly Training Home — a milestone series, renamed from "Weekly Confidence Loop" 2026-06-03 per `D149`)** remains the gate to any coach-connected work; it is **not** part of M001 tiering. M002 is now scoped as a series — a progression spine (M002.1 thin-spine + visible adaptation → M002.2 stress ladders + technique-how depth → M002.3 objective "1% better" drill score) plus attaching tracks (goals/anchor, bring-others-in, attack + tactics content) plus a Phase 1.5 handoff. The **coach gate opens only after the spine (M002.1–M002.3)** proves main-tool pull, not after M002.1 alone. **Series status (2026-06-12): M002.1 shipped 2026-06-04/05 (`D150`–`D152`); the stress-ladder substrate and default-path steering shipped ahead of M002.2 as pull-forwards (`D154` ladders + derived position + rung-steered assembly, `D157` stress visibility, `D159` steering on the default Setup path, `D160` catalog-wide ladder membership) — M002.2 is now a content milestone over an existing substrate.** Milestone-level breakdown: `docs/brainstorms/2026-06-03-m002-thin-spine-and-milestone-series-requirements.md`; series rationale: `docs/research/2026-06-02-m002-evidence-meta-synthesis.md`.
...
- Minimal weekly receipt: planned-vs-completed sessions, one load proxy (session RPE x minutes), one skill proxy; framed as a confidence and investment layer, not an analytics dashboard *(as shipped, this landed behavioral-primary per `D149`–`D152`: a consistency headline merged into the Recent-sessions header, no user-facing load proxy, and no persisted "planned" denominator under `D150` derive-don't-persist)*
...
Goal: deepen self-coached longitudinal value, give users durable data ownership, and sharpen adaptation once the Weekly Training Home layer (`D149`) is working.
