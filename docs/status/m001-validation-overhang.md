...
**Polish (Tier 2):** **unlocked 2026-05-23** per the 2026-05-21 `D130` Condition 3 final read-out passing in the week-5 catch-up entry (`docs/plans/2026-04-20-m001-adversarial-memo.md` `## 2026-05-18 (week 5 of 13)`). Tier 2 surfaces — "See why this session was chosen" modal, richer summary copy, full session history screen, recommendation-first onboarding polish — become pickable. Each pick still requires its own plan + A3 fresh check; the gate unlock is a permission, not an assignment.

## Calendar-Dated Reads

| Date | Gate | Source | Reads from |
| --- | --- | --- | --- |
| Every Monday | Adversarial-memo Weekly trigger-review ritual + dream pass | `docs/plans/2026-04-20-m001-adversarial-memo.md` lines 126-164 (Weekly trigger-review ritual + recognized behavioral-evidence channels) + `scripts/dream.sh` (alarm-clock script) | Founder-use ledger Sessions table + recent commits + A3 ligament status. **Three consecutive missed Mondays = early `D130` re-eval at the next weekly slot** (memo line 154). Last entry: `## 2026-05-18 (week 5 of 13)` (catch-up performed 2026-05-23, 5 days late; consecutive-miss clock at 1 of 3). Next ritual: 2026-05-25 (week 6) — normal cadence via `bash scripts/dream.sh`. Step 1: `bash scripts/dream.sh`. Step 2: hand-execute DDL. Step 3: append weekly-log entry. Step 4: delete dream prompt file. |
| Every >50-line plan or research edit (continuous) | A3 ligament — founder memo re-read within preceding 7 days | `docs/plans/2026-04-20-m001-adversarial-memo.md` lines 214-222 (`A3 — Beach Prep Three for the founder`) | Most recent `A3 re-read YYYY-MM-DD — <n> min` line in the memo's Weekly Log. Agents authoring on the founder's behalf must surface the check before proposing a new plan/research file. **Last read: 2026-05-23 (founder-confirmed closure recorded in week-5 catch-up entry; 13-day staleness gap from prior 2026-05-10 read recorded as agent disclosure, not erased — next read mandatory before any new plan/research file lands or any >50-line plan/research edit, with a 7-day staleness window; next staleness date 2026-05-30).** |
| 2026-05-12 | `D134` Phase 2A optional-streak falsification gate read-out | `docs/research/founder-use-ledger.md` "Bounded D130 exceptions and falsification gates" section | **READ 2026-05-23 (catch-up): CLEAN.** Zero sessions in the 2026-04-28 → 2026-05-12 window ran a streak drill (`d38-pair`, `d01-pair`, `d41-pair` at `main_skill` / `pressure`); both falsification gates' floors are 0; neither can fire. Phase 2B per-drill capture shapes (`points-to-target` / `pass-grade-avg` / `composite`) become pickable per `docs/status/post-m001-content-backlog.md` §"Conditional Phase 2B Capture Shapes." Picks still require their own plans + the standard founder-trigger conditions. See `docs/plans/2026-04-20-m001-adversarial-memo.md` `## 2026-05-18 (week 5 of 13)` D134 read for the full reading. |
| 2026-05-21 | `D130` Condition 3 final read-out (partner unprompted-open within 30 days of Tier 1a walkthrough) | `docs/plans/2026-04-20-m001-adversarial-memo.md` Condition 3 + the 2026-04-22 Amendment Log entry | **READ 2026-05-23 (catch-up): PASS.** 30-day quiet-window 2026-04-21 → 2026-05-21 demonstrated partner-return repeatedly: 2026-04-22 T+1 message + 2026-04-23 T+2 Dexie-instrumented open + five joint pair sessions across the window + 2026-05-10 reflection addendum (a) Seb-solo opens beyond T+1/T+2 + positive partner signal on 2026-05-13 `GlossedText` affordance landing. Quiet-window invariants held throughout. **Consequence: Tier 2 polish unlocks** (see Phase Posture above). Validation Posture String moves from L2 to L3. See `docs/plans/2026-04-20-m001-adversarial-memo.md` `## 2026-05-18 (week 5 of 13)` Condition 3 read for the full reading. |
...
- **Condition 1 — solo / set focus usage.** ≥40% solo-share + ≥3 set-focused sessions across the founder-use window. Evidence channel: founder-use ledger rows (`docs/research/founder-use-ledger.md`) + the four other channels per `D135` (chat / voice memo / partner walkthrough / Dexie export). Current state (2026-05-23): unchanged from week-4 — fail-trending under standard read (solo 1/7 ~14%; set-focused 0/3); ambiguous under `D132` pair-first / solo-accommodating re-read; do not amend until a dated decision pass.
- **Condition 2 — zero outside-app planning.** Founder did not plan sessions outside the app during the window. Evidence channel: founder self-report, weekly Monday review attestation. Current state (2026-05-23 attestation): **PASS for 2-player scope.** Founder attested 2026-05-23: 3s/4s outside-app planning happened a couple times (forced by `D101` content gap; does NOT count against Condition 2 under M001 scope); zero outside-app planning for 2-player sessions the app is designed to host. **Watch-item:** founder reports "thinking about" outside-app planning for 2-player attack/tactics content (F2/F5 near-displacement leading indicator) — Condition 2 stays PASS but the indicator is recorded for re-eval. See `docs/plans/2026-04-20-m001-adversarial-memo.md` `## 2026-05-18 (week 5 of 13)` Condition 2 read for the full reading.
- **Condition 3 — partner unprompted-open within 30 days of Tier 1a walkthrough.** Evidence channel: partner Dexie export + partner unprompted-open observation. Current state (2026-05-23 final read): **PASS.** 30-day quiet-window 2026-04-21 → 2026-05-21 demonstrated partner-return repeatedly: 2026-04-22 T+1 message + 2026-04-23 T+2 Dexie-instrumented open + five joint pair sessions across the window + 2026-05-10 reflection addendum (a) Seb-solo opens beyond T+1/T+2 + positive partner signal on 2026-05-13 `GlossedText` affordance landing. **Consequence: Tier 2 polish unlocks** (see Phase Posture above). Validation Posture String moves L2 → L3.
...
Current state (2026-05-23 week-5 catch-up read): no early triggers fired. Trigger (d) UP on the clock at 7/10 strict-ledger founder/joint sessions with 0 partner-walkthrough P0s (behavioral-reality count from the founder 2026-05-23 "dozen or more" attestation is likely already above 10; trigger reads strict ledger). Trigger (e) **reversed from week-4 "closest to firing" to clean** per founder 2026-05-23 direct attestation that dozen+ sessions of independent app use happened with all Seb feedback from real field experience. Triggers (a) / (b) / (c) clean. Read at every Monday review.
...
Current state (2026-05-23 catch-up read): **CLEAN.** The 14-day window 2026-04-28 → 2026-05-12 contained zero sessions where a streak drill ran; both falsification gates' floors are 0; neither can fire. **Phase 2B per-drill capture shapes (`points-to-target` / `pass-grade-avg` / `composite`) become pickable** per `docs/status/post-m001-content-backlog.md` §"Conditional Phase 2B Capture Shapes." See `docs/plans/2026-04-20-m001-adversarial-memo.md` `## 2026-05-18 (week 5 of 13)` D134 read for the full reading.
