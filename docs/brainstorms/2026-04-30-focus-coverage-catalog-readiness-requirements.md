...
- R25. The audit report should be scan-first: top summary by focus, failing cells grouped by visible user risk, then per-cell details with linked gap cards. Risk buckets are `cannot_generate`, `off_focus_support`, `no_same_focus_swap`, `thin_long_session`, `cannot_generate_at_level`, and `source_trace_missing`.
