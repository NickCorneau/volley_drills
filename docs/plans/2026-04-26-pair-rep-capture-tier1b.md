...
- **Re-trigger condition for Framing C.** If the founder logs ≥2 additional sessions after `D133` ships where the per-drill Good/Total optional card *also* felt fake or was systematically skipped on a session where the data was wanted, re-evaluate Framing C (in-session counter on RunScreen) under a separate plan. Do **not** retroactively reshape this plan; treat the next decision as forward-only. **Status (2026-04-27): 1 of ≥2 hits accumulated** — cca2 dogfeed (founder + Seb pair pass session) returned a "not sold on counting passes because of memory/awareness" report. One more session with the same flag fires the re-trigger formally.
