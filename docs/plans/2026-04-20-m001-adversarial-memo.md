...
**Recognized behavioral-evidence channels (2026-04-22-c + 2026-04-24 expansion + `D135` sharpening 2026-04-28).** Five channels feed the weekly read; no single channel's emptiness falsifies founder-use mode on its own, and the channel-mix read is what the 2026-07-20 re-eval consumes:
...
3. **Partner-walkthrough observations** — `docs/research/partner-walkthrough-results/*.md`, including joint founder+partner sessions flagged by their joint-session-reframe clarifiers, and partner voice memos / notes / Dexie exports paired with debrief commentary delivered against the production build. Per `D135` (2026-04-28), the test is source-validity (real session, real build, real observation), not scripted-vs-unscripted format.
4. **Condition 3 signals** — the 30-day unprompted-open tracker in the walkthrough ledger.
5. **Founder chat / voice-memo feedback to the repo and agent** — added 2026-04-24, sharpened by `D135` (2026-04-28). Substantive founder corrections, scope-discipline interventions, canonical framing changes, and **content-gap reports tied to real use sessions** delivered via chat (e.g., the 2026-04-22-c correction, the 2026-04-23 closeout-polish plan request, the 2026-04-24 joint-session reframe, the 2026-04-21 "we don't get to pick focuses" gap, the 2026-04-26 "fake count" report that fired `D133`'s `P2-3`). These are legible through agent transcripts + the downstream canon edits those transcripts produced; they are not separately stored as a "chat ledger" and do not need to be. **Boundary (per `D135`).** Founder content-gap reports ("on session X I tried Y and couldn't find Z") are first-class trigger evidence; founder feature wishes ("I think we should add a focus picker") are *input* to founder reasoning, not evidence trigger structures should treat as a hit. The boundary preserves the adversarial-memo concern that founder-use mode has no adversary in the room by default — content-gap reports describe observed friction the trigger structure was designed to capture; stated wishes without underlying friction would let the founder fire any trigger by saying anything.
...
- **F8 collapse reversed.** Phase F8 (2026-04-19, `docs/archive/plans/2026-04-19-feat-phase-f8-typography-foundation-plan.md`) deliberately collapsed the four mid-session slot types to `Work` on calm-voice / shibui-direction grounds. The cca2 dogfeed F1 evidence ("a 25-min session with three different work-block types and the user can't tell them apart from the eyebrow; only the main_skill block got the post-block Difficulty chip and the chip-asymmetry felt arbitrary because the role wasn't visible upstream") is the cited reversal evidence. The reversal is **not a thesis change** — sentence case kept, accent-color kept, no all-caps reintroduced, no new chrome — it is a label-set expansion under courtside legibility evidence. F8 plan doc is unchanged for now (the reversal logic lives here in the amendment log + in the `phaseLabel` source comments); future F8-class typography passes can cite this entry.
...
### Amendment log entry for 2026-04-28 build-17 dogfeed routing pass

**2026-04-28 — Build-17 founder + Seb pair dogfeed (15 min, no net, RPE 5, completed) routed across roadmap, milestone, and tier-plan evidence trackers (evidence-only, no scope change).** A 15-minute pair pass round on build 17 produced one new ledger row (`docs/research/founder-use-ledger.md` 2026-04-28), one curated research note (`docs/research/2026-04-28-build17-pair-dogfeed-feedback.md`), and a small sweep across `docs/roadmap.md` Tier 1c + audio-suspend risk language, `docs/milestones/m001-solo-session-loop.md` Tier 1c trigger-evidence note, `docs/milestones/m002-weekly-confidence-loop.md` history-boundary clarifier (history is owned by M001 Tier 2, not M002), `docs/status/current-state.md` Recent Shipped History, `docs/plans/2026-04-26-pair-rep-capture-tier1b.md` Framing-C re-trigger optionality update, and `docs/plans/2026-04-20-m001-tier1-implementation.md` Tier 1c + skill-level-mutability evidence rows.

**Justification.** Editorial / status class. Zero new drill records authored (cap consumed: still 4/10 from Tier 1b Layer A). No Dexie migration, no spec contract change, no falsification-condition change, no threshold shift, no decision-rule change, no `D###` row added. The pass *records* evidence accumulating; it does not *trigger* any tier or surface. Co-signer requirement waived per the amendment rule.

**Trigger-status updates from this pass:**

- **Tier 1b Framing C re-trigger (in-session running rep counter on `RunScreen.tsx`).** Pre-2026-04-28: 1 of ≥2 hits (cca2 dogfeed "not sold on counting passes"). Post-2026-04-28: **still 1 of ≥2 hits, plus countervailing optionality validation.** Build-17 returned Seb's positive read that the option to count was useful precisely because counting was *not* forced. One more session with the fake/skipped-count flag still fires the re-trigger formally, but **any future Framing C design must preserve the optionality constraint** as a pre-registered design rule. Recorded in `docs/plans/2026-04-26-pair-rep-capture-tier1b.md` §"Re-trigger condition for Framing C."
- **Tier 1c trigger (focus-picker / `sessionFocus` routing architecture).** Pre-2026-04-28: founder-side ledger evidence at 1 row (≥8 needed) plus partner-side cca2 voice-memo asks (arguably-equivalent walkthrough hit). Post-2026-04-28: **partner-side dogfeed asks now repeat across two sessions** (2026-04-27 cca2 + 2026-04-28 build-17). Strict reading of the ≥8-session OR clause is unchanged; the partner-walkthrough OR clause remains "arguably met" via *two* dogfeed voice memos rather than one. The founder reading at this update is still to keep the strict ≥8-session count as the safest bar, *not* to re-classify dogfeed voice memos as scripted-walkthrough equivalents — re-classifying would require a `D###` policy row, which this pass intentionally does not author. Updated language in `docs/plans/2026-04-20-m001-tier1-implementation.md` §"Tier 1c trigger evidence accumulating," `docs/milestones/m001-solo-session-loop.md` Tier 1c bullet, and `docs/roadmap.md` Phase 1 Tier 1c bullet.
- **Skill-level mutability (sibling surface, separate trigger).** Pre-2026-04-28: 1 partner-side ask, 0 founder-side hits. Post-2026-04-28: **2 partner-side asks, 0 founder-side hits.** Still below the strict scripted-walkthrough / founder-ledger trigger; record only. Recorded in `docs/plans/2026-04-20-m001-tier1-implementation.md` §"Skill-level mutability — separate surface, separate trigger."
- **Audio-pacing reliability (residual-risk channel under `docs/roadmap.md` screen-lock audio-suspend bullet).** Pre-2026-04-28: 2026-04-24 wake-lock + audio-primer mitigation shipped. Post-2026-04-28: **regression-shaped signal** — Seb reported missing beeps for sub-block ticks and the cooldown phase, with him watching the timer rather than relying on audio. This is routed as an **investigation step** (confirm device state, Wake Lock acquisition path, sub-block tick metadata coverage, cooldown-phase audio cue presence) under the existing residual-risk bullet, *not* as a new feature scope. No code change in this pass; no new plan authored. Updated language in `docs/roadmap.md` §"Risks and mitigations" screen-lock audio-suspend bullet.

**Behavioral-evidence channel mix this pass (per `docs/plans/2026-04-20-m001-adversarial-memo.md` §"Recognized behavioral-evidence channels", expanded 2026-04-24).** This pass advanced channel (b) partner usage and channel (c) partner-walkthrough observations (the build-17 voice memo is structurally a walkthrough Task-B debrief on a real session). Channels (a) founder-ledger sessions, (d) Condition 3 signals, and (e) founder chat / voice-memo feedback remained at their prior levels. The channel mix is not silent; the research-velocity-substitution failure mode (`docs/roadmap.md` §"Risks and mitigations") is not in play.

**Re-eval impact.** None. The Condition 3 final read date (2026-05-21) is unchanged. The 2026-07-20 re-eval decision rule is unchanged. The three falsification conditions are untouched.

**Validation.** `bash scripts/validate-agent-docs.sh` clean expected; JSON syntax check clean expected for `docs/catalog.json`; `git diff --check` clean expected on edited docs. No app code changes in this pass, so no `npm` verification is required.

### Amendment log entry for 2026-04-28 D135 walkthrough-equivalence policy clarification

**2026-04-28 — Walkthrough-equivalence policy clarification + Tier 1c trigger fire + skill-level-mutability sibling trigger fire (`D135`).** Founder direction in chat (2026-04-28, same agent session that landed the Phase 2A streak ship): "we should never require scripted walkthroughs anymore, lets use voice memos and any notes as real feedback if its not from me (and if its from me that still should count heavily imo, since this app is for me too)." Recorded in `docs/decisions.md` as `D135` with the three-clause policy (source-validity-gated walkthrough-equivalence; first-class founder evidence; content-gap-vs-feature-wish boundary). This memo's §"Recognized behavioral-evidence channels" sharpened in the same pass to (a) reflect that channels (b) and (c) include voice memos / notes / Dexie exports paired with debrief commentary regardless of scripted-vs-unscripted format, and (b) clarify that channel (e) founder content-gap reports tied to real use sessions count as first-class trigger evidence (not just framing input), with the explicit boundary that founder feature wishes are *input* to founder reasoning, not trigger evidence. Trigger-state consequences propagated across `docs/plans/2026-04-20-m001-tier1-implementation.md` Tier 1c trigger block (clause 2 fired by the 2026-04-27 cca2 + 2026-04-28 build-17 voice memos), the same plan's §"Skill-level mutability — separate surface, separate trigger" Status block (partner-walkthrough OR clause fired by the same two memos directly naming skill-level inflexibility), `docs/plans/2026-04-26-pair-rep-capture-tier1b.md` Framing C re-trigger language (evidence-source clarification, no count change), `docs/milestones/m001-solo-session-loop.md` Tier 1c bullet, `docs/roadmap.md` Phase 1 Tier 1c bullet, `docs/research/founder-use-ledger.md` opening prose, `docs/status/current-state.md`, and `docs/catalog.json`.

**Justification.** Policy-clarification class. Zero new drill records authored (cap consumed: still 4/10 from Tier 1b Layer A). Zero Dexie migration. No spec contract change. **No falsification-condition change.** The three pre-registered falsification conditions never used "scripted" anywhere — Condition 1 measures solo-share % and set-focused session count, Condition 2 measures outside-app planning count, Condition 3 measures partner unprompted-open count within 30 days; all three remain unchanged. The authoring-budget cap is unchanged. The 2026-07-20 default-against-inertia decision rule is unchanged. The fifth D130 trigger (agent-assisted open asymmetry) is unchanged. The Validation Posture String ladder (L1/L2/L3) is unchanged. No threshold shift, no decision-rule change. Co-signer requirement waived per the amendment rule (the rule binds only on weakening edits; this is a clarification of how to read existing triggers, not a relaxation of a falsification condition).

**Why this clarification exists.** Two compounding observations forced the call. First, the "scripted walkthrough" qualifier in the Tier 1c trigger evidence block was the founder's own conservative gloss appended to the trigger reading on 2026-04-27 / 2026-04-28, not part of the original 2026-04-20 trigger language — the original clause says "Partner walkthrough ledger contains a ≥P1 flag" with no scripted-vs-unscripted qualifier. Continuing to read "scripted" into the trigger meant the founder was holding a shifting bar against the partner-side evidence the trigger was designed to capture. Second, founder-side evidence was being implicitly downweighted in the same pattern: ledger rows counted toward Condition 1 and the ≥8 founder-session clause of Tier 1c, but founder-chat-delivered content-gap reports (the 2026-04-21 "we don't get to pick focuses," the 2026-04-26 "fake count" report that fired `D133`'s `P2-3`) were sometimes treated as procedural input rather than first-class trigger evidence — even though `D130` mode is *defined* as founder-use being the evaluation channel. The boundary clause (content-gap reports vs feature wishes) preserves the adversary the memo was authored to install: under a too-broad reading where any founder chat counts as evidence, the founder could fire any trigger by saying anything.

**Trigger-state updates from this clarification:**

- **Tier 1c trigger** (`docs/plans/2026-04-20-m001-tier1-implementation.md` §"Tier 1c trigger") clause 2 — partner walkthrough ≥P1 flag — **fired** under `D135`. Implementation is its own future ship under sequencing option A.
- **Skill-level mutability sibling trigger** (same plan §"Skill-level mutability — separate surface, separate trigger") partner-walkthrough OR clause — **fired** under `D135`. Implementation is its own future ship, separate from Tier 1c even though both fired the same day.
- **Tier 1b Framing C re-trigger** (`docs/plans/2026-04-26-pair-rep-capture-tier1b.md`) count is **unchanged** at 1 of ≥2 hits; the policy clarification only adds an explicit evidence-source list (founder ledger + founder chat / voice memos + partner voice memos / notes / Dexie exports paired with debrief).
- **`D130` early triggers (a)–(e)** — all unchanged. None depend on scripted-vs-unscripted distinctions; they read founder-session cadence (a, b), scope-expansion behavior (c), shipped-tier-plus-session-count (d), and agent-assisted-open asymmetry (e).
- **Falsification Conditions 1, 2, 3** — all unchanged.

**Re-eval impact.** None. The Condition 3 final read date (2026-05-21) is unchanged. The 2026-07-20 re-eval decision rule is unchanged. The three falsification conditions are untouched. The default-against-inertia outcome at 2026-07-20 (option (a) friends-of-friends if Conditions 1, 2, 3 all pass and Tier 1a + Tier 2 have shipped and combined sessions >12) is unchanged.

**Validation.** `bash scripts/validate-agent-docs.sh` clean expected; JSON syntax check clean expected for `docs/catalog.json`; `git diff --check` clean expected on edited docs. No app code changes in this pass, so no `npm` verification is required.

### Amendment log entry for 2026-05-10 Tier 1b slot expiry contract

**2026-05-10 — Tier 1b slot expiry contract landed (cap-discipline class).** Per `docs/plans/2026-05-10-005-feat-tier-1b-slot-expiry-contract-plan.md`, the 6 unconsumed Tier 1b drill-cap slots now carry a structured kill-or-author contract with a hard expiry of 2026-07-20 (D130 founder-use window close). Structured slot records and a machine-readable `<!-- cap-status-data:start -->` JSON block landed in `docs/status/post-m001-content-backlog.md`. A new validator function `cap_status_must_be_consistent` in `scripts/validate-agent-docs.sh` enforces structural integrity (status vocabulary `reserved` / `authored` / `killed`; sum check; required fields; expired-reserved-slot hard failure post-expiry). Test fixtures landed in `scripts/test-validate-agent-docs.sh` (13 new fixture-driven cases). The 4 currently-named reserved slots are recorded with their existing cited triggers; 2 unallocated reservation slack slots are made explicit (`t1b-unallocated-5`, `t1b-unallocated-6`). Cross-reference added to `docs/status/m001-validation-overhang.md` Calendar-Dated Reads 2026-07-20 row.

**Justification.** Cap-discipline class — structures existing reservation state, does not author or kill any slot. The Tier 1b cap value (10), anti-displacement enforcement, and per-slot trigger conditions are unchanged. D135 source-validity gating is unchanged. The contract is the no-trigger half of D135: absence of evidence must be cited and dated, not silent. Zero new drill records authored (cap consumed: still 4/10 from Tier 1b Layer A). No spec contract change. No Dexie migration. **No falsification-condition change.** Condition 1 thresholds, Condition 2 threshold, and Condition 3 30-day partner unprompted-open window all remain unchanged. The 2026-07-20 default-against-inertia decision rule is unchanged. Co-signer requirement waived per the amendment rule (the rule binds only on weakening edits; this is structural discipline added to existing reservation state, not a relaxation of any falsification condition).

**Trigger-state updates from this contract:** None — the contract converts the *recording surface* for trigger-status, not the trigger conditions themselves. Each of the 4 named reserved slots inherits its existing trigger citation (`t1b-pair-opening-block` → partner-walkthrough ≥P1; `t1b-pair-role-swap-cue` → partner-walkthrough ≥P1 OR founder ≥2 unclear-role-transition sessions; `t1b-rep-counter` → ≥2 founder-ledger / partner-walkthrough hits, currently 1 of ≥2; `t1b-stretch-demo` → partner-walkthrough ≥P1 OR ≥2 founder content-gap reports under D135). The 2 unallocated slots inherit D135 source-validity gating as the general trigger floor for any new Tier 1b drill-record-shaped need.

**Re-eval impact.** None on Condition 3's 2026-05-21 close date. Direct positive impact on the 2026-07-20 re-eval read: cap-status state becomes machine-readable and structurally consistent — "cap held 4/10 with 6 logged kill-or-author decisions citing source evidence" replaces "cap held 4/10 with 6 silent reservations" as the readable surface. Hard failure post-expiry on any remaining `reserved` slot forces a real decision rather than allowing indefinite drift.

**Validation.** `bash scripts/test-validate-agent-docs.sh` clean (42 cases, 13 new); `bash scripts/validate-agent-docs.sh` clean on the actual repo state. No app code changes, so no `npm` verification is required.
...
- **Authoring-budget cap check (2026-04-24 closeout polish).** 0 new drill records authored in the 2026-04-23 polish ship. Cap consumed: still 0 / 10 until founder's 5th post-Tier-1a session (per the ship log in `docs/archive/plans/2026-04-23-walkthrough-closeout-polish.md` §"Ship log"). Per the 2026-04-24 joint-session reframe, the founder is 2 sessions closer to that cap-gating 5th session than the ledger previously implied.
...
## 2026-05-11 (week 4 of 13)

*Ritual performed Sunday 2026-05-10 (one day early per founder ask after the missed-Monday discovery — see "Missed weeks" sub-section below). Reads the week-of-2026-05-04 + cumulative state. Plan path: `docs/plans/2026-05-10-001-docs-monday-ritual-week-4-plan.md`.*

- **Sessions logged this past week (week of 2026-05-04):** 2 (the 2026-05-04 pair mixed session with Seb, RPE 5, 40 min; plus the 2026-05-10 Pair + Net serve-focused session with Seb, actualDurationMinutes 38, RPE 5). Running total: 7 / 13-week target. Pace: under-target (target ≈10 by week 4 if the implied 1-per-week+ baseline holds; actual 7).
- **Sessions that actually ran this past week but are not yet in the ledger:** none known after the 2026-05-10 export update from a flagged-events standpoint. **Founder self-report (2026-05-10):** *"seb and i still use it at least 2x weekly (for every practice, and we like it)."* The 2x-weekly pair-practice cadence is real and routine; those routine sessions are NOT being logged as separate ledger rows because they're cadence, not flagged events. The ledger materially under-counts actual product-use cadence. **Captured durably** in `docs/research/founder-use-ledger.md` under the new 2026-05-10 cross-session reflections block (committed alongside this entry). No backfilled session rows added — that would violate the ledger's "never backfilled from memory more than 24h later" rule for specific dated rows.
- **Solo share so far:** 1 of 7 = ~14%. **Below the ≥40% bar** under standard `D130` Condition 1 reading. Apply `D132` re-reading (Condition 1 measures whether the *accommodated solo case works*, not founder solo dominance): the solo-case accommodation is shipped (Tier 1a + Tier 1b Layer A + Tier 1c). The re-read is **fail-trending on set-floor** (0 of 3 — see next bullet); **ambiguous on solo-share** under `D132`. The 2026-04-24 addendum's Seb-side cross-reference (3 of 4 partner sessions solo = 75%) reinforces the pair-first / solo-accommodating frame is what the build actually serves.
- **Focus breakdown so far:** pass 5 / serve 1 / set 0 / mixed 1 (the 2026-05-04 mixed session was serving-led but included a set swap; the 2026-05-10 session is counted as serve because main_skill and pressure were serving). Set-focused floor: **0 of 3 — failing**, sharper than the solo-share constraint. The 2026-04-21 founder ledger gap 3 ("session variety already being a bit of a concern" + serving-specific ask), the 2026-05-09 reflection's 3+ player gap, and the 2026-05-10 serve-focused pair session indicate the founder is steering toward pair / serve contexts, not toward set-focused 2-player work. Reading it honestly: the set-floor was set against assumed solo/pair set practice patterns the founder does not actually pursue. This may be a `D132`-style re-read candidate at 2026-07-20 — but no amendment now; surface only.
- **Outside-app planning this week:** `[founder self-report needed]` — no observable evidence of outside-app planning in repo or chat. Founder closure removes Condition 2 ambiguity.
- **D130 early triggers check:**
  - **(a) <5 sessions in 45 days:** 7 founder/joint sessions across 20 days elapsed (2026-04-21 → 2026-05-10) — pace OK; not tripped. Window remains 25 of 45 days.
  - **(b) 3-week silence:** most recent session 2026-05-10 — not tripped. Trigger fires at 21+ days; clock reset by today's export.
  - **(c) invited anyone outside partner:** **soft yes, hard no.** Founder self-report (2026-05-10): *"i have shown it to a couple people but since no one is in 2s no one has used it yet."* Trigger (c) text (memo line 192) reads "invites an external user to USE the app." **Show ≠ use.** Trigger **not firing as defined.** The soft scope-leak signal is worth carrying forward: non-Seb people are now aware of the app but haven't tried it because the build doesn't fit their training configurations. This sharpens the 2026-05-09 reflection's 3+ player content-gap evidence — the cohort to leak to barely exists in the founder's network at the build's current scope, which is structural (cohort-availability, per dream-pass Fragment 2 above), not discipline failure. **Trigger semantics note for the 2026-07-20 re-eval:** the "show but no use" outcome is the exact pattern dream-pass Fragment 2 predicted; reading trigger (c) at re-eval should weigh "shown, no uptake, structural cohort mismatch" as evidence routing toward `D101` 3+ player support sequencing rather than reading "no uptake" as a `D91` cohort-hunt failure.
  - **(d) Tier 1a + ≥10 sessions + no open P0:** Tier 1a + Tier 1b Layer A + Tier 1c shipped; founder/joint sessions 7 / 10; partner walkthrough 0 P0 — on the clock; needs 3 more sessions before firing.
  - **(e) agent-assisted open asymmetry:** ≥5 agent-assisted opens in last 14 days — **strongly yes** (≥80 commits 2026-04-26 → 2026-05-10, all agent-authored per the dream-pass git-log chunk). Agent-free check: two ledger-logged founder + Seb sessions (2026-05-04 and 2026-05-10) + the 2026-05-09 ledger reflection (founder-authored content-gap report) + the 2026-05-10 founder self-report of routine 2x-weekly pair-practice use ("for every practice, and we like it") constitute substantial agent-free product behavior. **Founder self-report (2026-05-10):** *"it is fully agent built and user used and founder led."* This is the **healthy version** of the founder-use mode (agent-as-builder, founder-as-user-and-leader), not the failure mode trigger (e) was designed to catch ("researched-velocity-as-substitute" per `docs/roadmap.md` Risks-and-mitigations — agent-talk-without-product-use). **Trigger (e) not firing.** Closed definitively for this read.
- **Non-ledger behavioral-evidence channels this week:**
  - Partner usage (Seb): second pair + net Dexie export landed 2026-05-10 (`volley-drills-export-2026-05-10.json`) with Seb session data and review note. This is partner usage evidence, not proof of unprompted open. `[founder self-report needed]` for any out-of-band Seb activity.
  - Founder chat / voice-memo feedback to repo: substantive — drove the 2026-05-08 Bucket B decisions (`D141`/`D142`/`D143`/`D144`), the 2026-05-08 Bucket A1 `SourceBackedReroute[]` registry refactor decision, the 2026-05-09 M001 build-complete relabel + new validation-overhang scoreboard, the 2026-05-09 ledger backfill posture, and the 2026-05-09 cross-session reflections (3+ player content gap as `D135` first-class evidence). Estimated ≥4 substantive chat threads landed canon edits this week.
  - Joint-session evidence: the 2026-05-10 Pair + Net session with Seb adds a second week-of-2026-05-04 joint session and strengthens the warmup / duration-budget risk.
- **Falsification conditions status:**
  - 1 (solo-first, ≥3 set-focused): **fail-trending under standard reading; ambiguous under `D132` re-reading.** Solo-share 14% (below 40%); set-floor 0 of 3 (the sharper sub-constraint). See bullet above; sharpened by 2026-05-09 reflection's 3+ player evidence and 2026-05-10 serve-focused pair use.
  - 2 (no outside-app planning): **pass** (no observable outside-app planning); founder self-report would close definitively.
  - 3 (partner unprompted-open within 30 days): **pending — provisional pass strengthened.** 30-day clock from 2026-04-21 walkthrough closes 2026-05-21 (11 days). Quiet-window invariants holding (URL live, no founder prompting). 2026-04-22 T+1-day Seb message + 2026-04-23 T+2-day Dexie-instrumented session = strengthened provisional pass per `m001-validation-overhang.md`; 2026-05-10 is additional partner usage evidence, not unprompted-open proof.
- **Validation Posture String (per A2):** **L2 — "Tested with one partner, 30-day quiet-window outcome pending/observed."** Walkthrough happened 2026-04-21; 30-day quiet-window clock closes 2026-05-21 (11 days remain), which is exactly the L2-pending state per A2's definition at memo line 209. Permits case-study framing with partner outcome explicitly named (Seb's T+1-day message + T+2-day Dexie-instrumented open). Still prohibits any retention-percentage claim. **Self-correction note (2026-05-10):** the week-4 entry initially pinned L1 with rationale "30-day partner quiet window has not closed" — that rationale inverted A2: the pending state IS L2 territory, not L1. L1 is the pre-walkthrough state. ce-correctness-reviewer P2 finding caught this at autofix time; corrected before commit. No external claim was issued under the wrong pin.
- **A3 ligament status:** **CLOSED 2026-05-10.** Founder confirmed memo re-read after the week-4 ritual surfaced the overdue chronic gap.
  - `A3 re-read 2026-05-10 — founder confirmed (per chat: "okay I just read A3"); duration not specified, estimated ≤5 min per memo line 220 self-estimate.`
  - The chronic gap (named 2026-04-24, persisted through weeks 2 and 3) is interrupted, not erased — the discoverability fix (Monday-ritual pointer added to `docs/status/m001-validation-overhang.md` Calendar-Dated Reads, committed alongside this entry) is the structural mitigation; this 2026-05-10 read is the first individual data point that the founder is engaging with the rule. Next read is mandatory before any new plan/research file lands or any >50-line plan/research edit, with a 7-day staleness window.
  - **Disclosure preserved (not retracted):** the week-4 plan (`docs/plans/2026-05-10-001-docs-monday-ritual-week-4-plan.md`, 250 lines) was authored by the agent on the founder's behalf without the agent surfacing the A3 check first. The 2026-05-10 closure does not retroactively un-violate that authoring — it confirms the rule is now active again going forward.
- **Authoring-budget cap check:** 0 new drill records authored this week. Cap consumed: 4 / 10 (Tier 1b Layer A: `d31`, `d33`, `d40`, `d42`). 5th-session gate not yet hit (currently at 6 founder/joint sessions, but the cap-gate counts post-Tier-1a founder sessions specifically — exact count at 4 by strict reading; reading partner-side joint sessions in is 6). No authoring this week, so the gate isn't tested.
- **One-line note:** Build is complete and validation gates are pending; the system is working as designed but the silent missed-weeks and overdue A3 are real discipline gaps the scoreboard discoverability fix lands today.

### Missed weeks 2 and 3 (2026-04-27, 2026-05-04) — recorded as data, not backfilled

The Weekly Log shows only the week-1 (2026-04-20) entry. Weeks 2 (2026-04-27) and 3 (2026-05-04) were silently skipped — no entries land, no addendums note the misses. **Per memo line 154** ("If three consecutive Mondays are missed, that is itself evidence the founder-use premise is underwater; trigger a re-eval at the next weekly slot"), the consecutive-miss count would have hit 3 if week 4 (2026-05-11) was also missed, firing an early re-eval at the next weekly slot (2026-05-18). The week-4 entry above is the not-missed third Monday; the consecutive-miss clock is interrupted at 2.

**Honest cause:** founder discoverability gap. The chat trail (2026-05-09 → 2026-05-10) shows the founder said "i wasn't aware of the Monday weekly ritual" when invoking this `/lfg`. The ritual is documented in this memo at lines 126-164 but is **not surfaced** from `AGENTS.md`, `docs/status/m001-validation-overhang.md`, or any agent-routing surface. This is a memo-discoverability finding, not a behavioral-falsification finding. The fix is structural: surface the Monday ritual from the validation-overhang scoreboard so future agents and the founder discover it from the scoreboard's "use this when" path, not only from this memo's body.

**Action landed in the same `/lfg`:** `docs/status/m001-validation-overhang.md` updated with a Monday-ritual pointer in the Calendar-Dated Reads + Agent Quick Scan sections (commit landing alongside this entry).

**Backfill not performed.** Weeks 2 and 3 are recorded here as missed, not retroactively filled in with synthetic Monday entries. Consistent with the memo's "ritual backfill that hides the missed-Monday signal" anti-pattern. Data backfill (e.g., 2026-04-27 / 2026-05-04 weekly rollups in the founder-use ledger) is OK and shipped 2026-05-09 (`256ad8b`); ritual backfill is not.

### 2026-05-10 — dream pass

#### Dream Fragments kept

- **Fragment 1 (RECOMBINATION: A ↔ B / cross-layer validation ↔ code).** The A3 ligament's enforcement mechanism is "the Weekly Log will show the last read-through date" (memo line 222). When the Weekly Log itself goes silent, A3 silently lapses — there's no surface that surfaces A3's lapse. The 2026-04-24 addendum already noted this once; weeks 2 and 3 confirmed the lapse is chronic, not one-off. **Concrete next action:** add A3 re-read status as a calendar-dated read on `docs/status/m001-validation-overhang.md` (or in the Agent Quick Scan), so it's discoverable independent of whether the Monday ritual itself is performed. Surfaces the "ritual depends on ritual" recursive failure.
- **Fragment 2 (RECOMBINATION: D ↔ F / cross-layer product ↔ meta-process).** The 2026-04-20 red-team flagged "perpetual-founder-use trap (P0)" as the failure mode where founder-use never converts to external validation. The 2026-05-09 founder reflection sharpens the *mechanism*: not founder discipline, but *cohort availability* — the founder's actual training network is "a lot of beach 3s/4s/6s" while the build supports 2-player. The trap may not fire because the founder won't invite people; it may fire because the people in the network aren't in configurations the build serves. **Concrete next action:** at the 2026-07-20 re-eval, read trigger (d) and (c) with an explicit "cohort-availability check" sub-question. If the founder's actual training network does not fit the build's supported configurations, "no founder-initiated invites" is structurally explained, not interpreted as discipline failure — and the consequence routes toward `D101` 3+ player support sequencing rather than a `D91` cohort hunt.
- **Fragment 3 (RECOMBINATION: E ↔ B / cross-layer docs ↔ code).** Several stale `docs/research/` candidates are stale because the *topic moved into code*, not because the topic was abandoned. `dexie-schema-and-architecture.md` was pre-implementation scoping; the schema is now v6 in `app/src/db/`. `srpe-load-adaptation-rules.md` rules are encoded in `D84` + sRPE primitive. The pruning-bar test ("never referenced from any active plan") **fails** for all 9 stale candidates this week — they're all referenced from canon. But the doc-set has a different drift risk: future agents reading the research as authoritative against schema-v6 will get drift confusion. **Concrete next action:** add `Status: superseded by <code path> as of <decision-id>` deprecation headers to research docs whose topic now lives in code (start with `dexie-schema-and-architecture.md` → `app/src/db/` schema v6; `srpe-load-adaptation-rules.md` → `D84` + load primitive). Soft move, not archive. Future-pass cleanup; not in scope for this `/lfg`.

#### Noise-injected fragment

- **Fragment 1 (sharpened):** The original framing was "A3 lapses silently when the Weekly Log goes silent." Push to contradiction: *what enforces A3 if the founder is the product, the user, AND the maintainer of the ritual?* No external party is in the room. The de facto enforcement layer is **the agent reading the memo when the founder asks "what's next."** The agent surfaced the missed-Monday gap on 2026-05-10 only because the founder asked "whats next." If the founder hadn't asked, week 4 would have been the third missed Monday and the agent would never have surfaced the early-re-eval trigger. **Sharpened action:** the agent's "what's next" routing must always check Monday-of-the-week against the Weekly Log's last entry, not just when the founder volunteers an interest in the ritual. Encode this as a routing rule in `AGENTS.md` "Operational Constraints" or as a `.cursor/rules/` entry. **Status:** proposal; founder co-sign on the routing-rule shape would normally precede landing it. Not in scope for this `/lfg`; surfaced for next-pass discussion.

#### Pruning proposals

- **Nothing meets the prune bar this week.** The 9 stale-doc candidates from the dream prompt's chunk E were all checked for "never referenced from any active plan"; **all 9 fail the test** (every candidate is referenced from `docs/catalog.json`, `docs/decisions.md`, `docs/roadmap.md`, `docs/research/README.md`, or active plans). The 21-day-untouched signal here is "stable canonical inputs that haven't needed refresh," not "abandoned scaffolding." The honest read: the repo's canonical surface is leaner than 21-day-untouched would suggest, and the dream-script's stale-doc heuristic over-flags by inclusion-not-by-reference. **No archive candidates land this week.** Fragment 3 above proposes a softer move (deprecation-header pointers on research-superseded-by-code docs) for next-pass cleanup; that is not the same as archive-to-`docs/archive/`.

#### Discarded fragments

- "The session-builder refactor velocity is silently consuming founder energy that would otherwise go to courtside dogfooding" — too rhetorical, no concrete new constraint, restates trigger (e) without adding sharpening.
- "The 2026-04-21 voice-memo channel that surfaced ledger gap 4 (timer-end audio suspension) should be formalized into a recurring channel" — founder would already do this when the next courtside finding fires; the dream doesn't add anything beyond what the existing channel-mix list captures.
- "Tier 1c focus picker is the load-bearing M001 surface that proves the post-`D132` accommodation thesis" — true but well-documented in the milestone doc; restates rather than adds.

#### A3 ligament status

- Last memo read-through: **none logged in Weekly Log.**
- Re-read required before next plan/research edit: **YES — currently overdue by definition.** Founder action: read memo end-to-end (~5 min) and append `A3 re-read YYYY-MM-DD — <n> min` as a sub-line under this week-4 entry. The week-4 plan (`docs/plans/2026-05-10-001-docs-monday-ritual-week-4-plan.md`, 250 lines) was authored without surfacing the check; agent disclosure recorded above.
