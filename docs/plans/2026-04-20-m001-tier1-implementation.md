...
| Full serving ladder (`d31`–`d37`, 7 rungs, float → jump-float gated) | Tier 1b (partial) | Capped at 10 total new drill records across Tier 1b per adversarial memo. **Layer A candidates (ships now, §R7 exit-3 applied 2026-04-26):** `d31 Self Toss Target Practice` (solo, rung 1), `d33 Around the World Serving` (solo + pair, rung 3). **Deferred from current Tier 1b Layer A:** `d36 Jump Float Introduction` (solo, jump-float entry) — re-enters under `O7` track 2 (sports-medicine / PT review), not via current Tier 1b. **Progression links:** use only authored same-chain drills: `d22` and `d31` can progress to `d33`, with `d22` → `d31` as a lateral no-net / ball-light entry-point alternative. No dangling `d32`, no current `d31` → `d23` link while `d23` is non-M001, and no cross-chain `d24` link. |
| Full setting chain (`d40`, `d42`, `d43`, `d44`, `d45`) | Tier 1b (partial) | Shared 10-drill cap with serving. Current Layer A candidates: `d40 Footwork for Setting` (solo), `d42 Corner to Corner Setting` (pair). **Deferred from current Layer A:** `d43 Triangle Setting` re-enters under `D101` 3+ player support because the BAB source geometry is 3-player, not a pair drill to force into M001. **Progression links:** rung 1 (`d38 Bump Set`), rung 2 (`d39 Hand Set`), and rung 3 (`d40 Footwork` once authored) are all default-unlocked — they are fundamentals, not rungs gated on each other; current pair progression is `d41` → `d42` only. |
...
- **Layer A candidates (4 drills, §R7 exit-3 applied 2026-04-26 + 2026-04-27 `d43` red-team correction):** `d31 Self Toss Target Practice`, `d33 Around the World Serving`, `d40 Footwork for Setting`, `d42 Corner to Corner Setting`. Authoring spec: `docs/plans/2026-04-22-tier1b-serving-setting-expansion.md`. Deferred from current Layer A: `d36 Jump Float Introduction` re-enters under `O7` track 2 (sports-medicine / PT review); `d43 Triangle Setting` re-enters under `D101` 3+ player support. Layer-A cap consumption is 4/10, not 6/10.
- Progression links for `chain-6-serving` and `chain-7-setting` with the dead-end fixes documented in the deferred-scope table above: serving uses only authored same-chain drills (`d22` / `d31` → `d33`, plus lateral `d22` → `d31`); setting fundamentals stay default-unlocked and current pair progression is `d41` → `d42` only.
...
**Trigger fired 2026-04-28 (clause 2, partner-walkthrough OR clause) under `D135`:**

- **Partner side (≥P1 hits across 2 dogfeed sessions — 2026-04-27 and 2026-04-28).** Seb's 2026-04-27 cca2 voice memo contained three structurally walkthrough-Task-B-class asks: (i) "today I want to do 30 minutes of serving — let me select that" (focus picker), (ii) "let me change my skill set level day-to-day" (skill-level mutability — *separate surface*, see below), (iii) "if I selected intermediate, let me choose what skills I want to drill down to" (focus picker × skill-level interaction). His 2026-04-28 build-17 memo repeated the "landing page" and "option to choose a new difficulty setting" point as his consistent ask. Recorded in `docs/research/2026-04-27-cca2-dogfeed-findings.md` F4 and `docs/research/2026-04-28-build17-pair-dogfeed-feedback.md` F4. Under `D135` (2026-04-28, walkthrough-equivalence is source-validity gated, not script gated), these two voice memos satisfy clause 2 of the Tier 1c trigger as originally written — the 2026-04-20 trigger language says "Partner walkthrough ledger contains a ≥P1 flag" with no scripted-vs-unscripted qualifier; `D135` removes the script-only gloss the founder had added to the trigger reading and the clause fires.
- **Founder side (corroborating, 3 hits, 1 session — 2026-04-21).** `docs/research/founder-use-ledger.md:90-94` recorded three independent gaps from the joint pass session: gap 1 ("can't pick focus in the UI"), gap 2 ("serving drills are not reachable via Swap on `solo_open`"), gap 3 ("session variety is already a concern on day 1 of Tier 1a use"). All three map to the focus-routing surface this section gates and qualify as first-class founder content-gap evidence under `D135` (real session, real build, observed friction — not stated wishes). Clause 1 of the trigger (≥8 founder-ledger sessions) is independently not at threshold (1 / 8); the trigger fires under clause 2 with clause 1 evidence corroborating.
- **Status.** Tier 1c trigger **fired** 2026-04-28 under `D135`. Implementation is its own future ship under sequencing option A — same precedent as `D133` (trigger fired 2026-04-26 → ship landed 2026-04-27 as the `/run/check` Drill Check surface). The §"Architectural prerequisites" block below remains the binding artifact for Tier 1c implementation; the founder retains the choice to bundle or sequence Tier 1c with the skill-level-mutability sibling surface (also fired 2026-04-28 under `D135` — see below) when those ships are scheduled. Strict-clause arithmetic preserved for the record: clause 1 (≥8 founder-session intent-mismatch ledger rows) at 1 / 8 — *not* the firing path; clause 2 (partner-walkthrough ≥P1 flag) **fired** under `D135`; clause 3 (≥3 set-focused Swap-friction sessions) at 0 / 3 — not the firing path.
- **Boundary preserved.** Per `D135`, founder feature wishes are not evidence; founder content-gap reports are. The 2026-04-21 ledger gaps 1–3 above are content-gap reports (real session, real build, observed friction). A founder chat saying "I think we should add a focus picker" without an underlying observed friction would not have fired the trigger under any clause; the boundary is preserved here so future agents reading this block do not generalize the policy past its stated scope.

### Skill-level mutability — separate surface, separate trigger

Seb's "change my skill set level to better reflect my skill set day-to-day" (cca2 dogfeed 2026-04-27) is **not** the focus-picker surface and should not be bundled into the Tier 1c plan. Today, `onboarding.skillLevel` is captured once at first-run (`storageMeta` row, value `'unsure'` in the export confirms it has never been re-prompted) and is read by the session builder only as a soft-tuning input. Mutability of skill level is a separate product surface:

- **Architectural shape.** A Settings-side "Adjust today's level" affordance, OR a draft-screen one-tap "level up / level down" button that mutates a per-session `skillLevelOverride?: 'beginner' | 'intermediate' | 'advanced'` (without touching the persistent `onboarding.skillLevel`). Either shape is small; both preserve `P11` recommend-first.
- **Trigger condition.** Founder-use-ledger logs ≥2 sessions with an explicit skill-level mismatch note ("today should have been easier / harder than the default"), OR partner walkthrough ≥P1 flag specifically about skill-level inflexibility, OR Tier 1c ships first and the focus picker reveals that focus-mismatch was masking skill-level-mismatch.
- **Status (2026-04-28).** **Trigger fired** under `D135` via the partner-walkthrough OR clause. Seb's 2026-04-27 cca2 voice memo carried "let me change my skill set level day-to-day" — a direct ≥P1 flag specifically about skill-level inflexibility — and his 2026-04-28 build-17 memo repeated the "option to choose a new difficulty setting" ask, landing on the same clause. Under `D135` (walkthrough-equivalence is source-validity gated, not script gated), these two voice memos satisfy the partner-walkthrough OR clause as written. Founder-side: 0 ledger rows naming explicit skill-level mismatch; the founder-ledger clause is independently not at threshold (0 / 2). Implementation is its own future ship under sequencing option A — separate from Tier 1c, even though both fired the same day, because the two surfaces answer different user asks (Tier 1c: "today's skill emphasis"; skill-level mutability: "today's load posture") and the architectural shape is separate (`sessionFocus?` field on draft context vs `skillLevelOverride?` field on draft context, with their own UI affordances). Per `D135` they may be bundled or sequenced at the founder's discretion when scheduling.

This is documented here so the eventual Tier 1c implementation ship does not absorb skill-level mutability by accident. The two surfaces share UI real estate (both touch the draft screen) but they answer different user asks ("today's skill emphasis" vs. "today's load posture") and should ship as distinct chunks even when scheduled close together.
