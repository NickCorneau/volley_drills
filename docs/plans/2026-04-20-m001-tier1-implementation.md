...
**Trigger evidence accumulating (2026-04-28 build-17 dogfeed read):**

- **Founder side (3 hits, 1 session — 2026-04-21).** `docs/research/founder-use-ledger.md:90-94` recorded three independent gaps from the joint pass session: gap 1 ("can't pick focus in the UI"), gap 2 ("serving drills are not reachable via Swap on `solo_open`"), gap 3 ("session variety is already a concern on day 1 of Tier 1a use"). All three map to the focus-routing surface this section gates.
- **Partner side (repeat asks across 2 dogfeed sessions — 2026-04-27 and 2026-04-28).** Seb's 2026-04-27 cca2 voice memo contained three structurally walkthrough-Task-B-class asks: (i) "today I want to do 30 minutes of serving — let me select that" (focus picker), (ii) "let me change my skill set level day-to-day" (skill-level mutability — *separate surface*, see below), (iii) "if I selected intermediate, let me choose what skills I want to drill down to" (focus picker × skill-level interaction). His 2026-04-28 build-17 memo repeated the "landing page" and "option to choose a new difficulty setting" point as his consistent ask. Recorded in `docs/research/2026-04-27-cca2-dogfeed-findings.md` F4 and `docs/research/2026-04-28-build17-pair-dogfeed-feedback.md` F4.
- **Strict trigger reading.** None of the three OR clauses formally fires: the ≥8-session ledger threshold is at 1, the partner-walkthrough trigger requires a *scripted* walkthrough (not dogfeed voice memos), and the ≥3-set-focused-Swap-friction count is at 0. The founder reading remains that **the partner-walkthrough OR clause is arguably met** because Seb's voice-memo asks are structurally identical to a Task-B debrief; the ≥8-session strict count remains the safest reading. **No tier change yet.** Strict count stands; Phase-1 readiness work documented in §"Architectural prerequisites" below stays the binding artifact whenever Tier 1c does fire.
- **Evidence accumulating** vs. **trigger met** is a deliberate distinction. The 2026-04-28 row in the founder-use-ledger records repeat partner-side pressure for focus/level entry surfaces; a scripted partner-walkthrough ≥P1 or ≥6 more founder-session intent-mismatch notes would formally fire the strict Tier 1c trigger.
...
- **Architectural shape (when triggered).** A Settings-side "Adjust today's level" affordance, OR a draft-screen one-tap "level up / level down" button that mutates a per-session `skillLevelOverride?: 'beginner' | 'intermediate' | 'advanced'` (without touching the persistent `onboarding.skillLevel`). Either shape is small; both preserve `P11` recommend-first.
...
- **Status (2026-04-28).** Evidence accumulating: 2 partner-side dogfeed asks across 2026-04-27 and 2026-04-28, 0 founder-side skill-level-mismatch rows. Below the strict scripted-walkthrough / founder-ledger trigger; record only.

This is documented here so Tier 1c, when it fires, does not absorb skill-level mutability by accident. The two surfaces share UI real estate (both touch the draft screen) but they answer different user asks ("today's skill emphasis" vs. "today's load posture") and should ship independently.
