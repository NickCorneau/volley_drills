...
## Post-Ship Code-Review Residuals (2026-06-23)

ce-code-review (7 reviewers) ran on the landed branch. All structured findings were applied before ship: the block-opening re-open bug (now prefix-scan first-appearance + interleave regression test), the `Now` cue + swap/shorten/skip lexicon single-sourcing, the trimmed `currentCue` comment, the stale AE test titles, and the doc syncs (D164 row cells, ideation frontmatter + catalog entry, current-state). The following are advisory `residual_risks` / `testing_gaps` deliberately **not** actioned this pass — tracked here, none ship-blocking:

- **R7 mid-block body census not enforced.** `TransitionScreen.beat-density.test.tsx` does presence/absence checks but is seeded at a block opening and does not pin a "mid-block body = {title, eyebrow, duration, full read, footer} only" budget the way `HomeScreen.render-budget.test.tsx` pins Home. Add a mid-block-seeded census if Transition density regresses.
- **Cross-surface guard scanner blind spots (intentional tradeoffs).** `runFlowLexicon.guard.test.tsx` joins text nodes with `\n` (a sunset phrase split across sibling element nodes would evade), scans only `aria-label`/`aria-description`/`title`/`alt`, and is case-sensitive. Realistic single-string reverts are caught; harden only if a real evasion appears.
- **Mid-run Swap can split the focus chain.** `swapAlternatives` strips `sessionFocus` when the focused pool is empty and re-picks cross-focus (documented intent-violation); a swap can land a different-focus drill mid-session. Pre-existing; rung-awareness of mid-run swap is already a named `D157`/`D159` deferral. The opening helper's first-appearance keying already prevents the *assembly-time* support-slot re-open.
- **Pre-start Swap intent recompute unverified.** No test asserts that a Transition pre-start Swap (live-query `nextBlock` refresh) recomputes `rungIntentLine` when the swapped-in drill sits on a different ladder/rung. Behavioral, low-risk; add a controller test if dogfood shows a stale line.
- **Discriminating-by-data fragility.** The mid-block suppression tests rely on `d20` sitting on pass rung 4 and `d41` on the set ladder; a future catalog move could silently degrade them to passing for the wrong reason. Acceptable given `D160` catalog-wide ladders + validation cross-checks.
- **`currentCue.fullInstructions` is pinned but has no production reader** until Stage 2's recovery peek (documented at the field). Delete + re-add if Stage 2 slips.
- **Capture-this follow-up (learnings reviewer):** the run-flow beat-contract + sunset-label + cross-surface-guard pattern is not yet a `docs/solutions/` learning; consider `/ce-compound` after the pattern proves out across a second run-flow change.
