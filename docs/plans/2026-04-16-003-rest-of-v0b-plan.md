...
depends_on:
  ...
  - docs/archive/plans/2026-04-16-001-feat-phase-a-schema-data-shape-plan.md
  - docs/archive/plans/2026-04-16-002-feat-phase-b-test-infra-sw-safety-plan.md
...
V0B-12 (drill-variant grain), V0B-23 (actualDurationMinutes), V0B-29 (borderlineCount round-trip). Plan: `docs/archive/plans/2026-04-16-001-feat-phase-a-schema-data-shape-plan.md`.

### Phase B — test infra and SW safety (complete)

V0B-20 (SW prompt flip + UpdatePrompt), V0B-21 (warm-offline smoke), V0B-22 (blocked schema-upgrade smoke + SchemaBlockedOverlay). Plan: `docs/archive/plans/2026-04-16-002-feat-phase-b-test-infra-sw-safety-plan.md`.
...
Plan: [2026-04-17-feat-phase-e-content-tooling-plan.md](../archive/plans/2026-04-17-feat-phase-e-content-tooling-plan.md). Vitest + Playwright clean: 61 files / 444 Vitest tests + 27 Playwright tests all green.
...
1. **`MediaSession` + `positionState` + `navigator.audioSession.type = 'ambient'` (iOS 17+).** The media-session card on iOS is a "Now Playing"-shaped tile — album-artwork box, title line, play/pause controls, and a progress bar we can drive per second via `setPositionState`. That progress bar *is* effectively a countdown, and the card updates even while the phone is locked. The platform requires an actual `<audio>` element playing a stream; our current Web Audio oscillator path in `app/src/lib/audio.ts` does NOT drive `MediaSession`, so this is new plumbing. The standard cost is audio-focus conflict with the user's music. The potential escape hatch is `navigator.audioSession.type = 'ambient'`, which tells iOS "present my metadata but don't take the audio channel away from other apps" — under-documented, under-tested, and varies across iOS minor versions.

   **Scoped spike (2-3 hours on real iOS hardware, post-D91):**
   - Build a `RunScreen`-only branch that: (a) lazily instantiates a near-silent `<audio>` element when a block starts (loop a <0.01 amplitude PCM buffer or a 1s silent .mp3); (b) sets `navigator.audioSession.type = 'ambient'` if the API is present; (c) sets `navigator.mediaSession.metadata` to `{ title: "Block N of M · ${drillName}", artist: "Volleycraft", album: "Session", artwork: [Volleycraft logo] }`; (d) calls `navigator.mediaSession.setPositionState({ duration, playbackRate: 1, position })` every second from the existing `useTimer` tick.
   - Acceptance criteria for continuing past the spike:
     - Card appears on the iPhone lock screen within 2s of starting a block.
     - Progress bar updates live (visibly advances each second).
     - Starting a block with Spotify already playing does NOT pause Spotify.
     - Locking the phone mid-block keeps the card visible and updating for ≥ 2 min (iOS suspension threshold).
     - Unlocking/app-foregrounding does not freeze the in-app timer (no double-source drift).
   - Abandon if ANY criterion fails. Card-look-is-Now-Playing is acceptable; music-interruption is not.

2. **Web Push notifications at block-end** (iOS 16.4+, installed PWA only). Lock-screen ping at block boundary — no live countdown, but a reliable "block ended" system notification even when the screen is off. Requires "Add to Home Screen" (fraction of testers do this) plus a notification permission grant (another gate on Safety screen, against the "answer-first, minimal gates" posture in `D86` / `V0B-16`). Plausible *complement* to foreground audio rather than a replacement. Cheaper and less fragile than route 1 if the evidence says testers only need the boundary ping, not a live countdown.

3. **Native iOS shell with a Live Activity / ActivityKit.** Only route to a true live lock-screen countdown with native-looking chrome; out of scope for PWA validation.

**Decision:** status quo for v0b. When the partner-walkthrough script (`docs/research/partner-walkthrough-script.md`) stabilizes after its current two-pass refactor, add one Pass 2 debrief question: *"Did you lock the phone during a session, and if so did you miss a block-end cue?"* — that single data point is what gates running the spike. If the D91 readout shows this is a common failure mode, run the scoped spike in route 1 first. If the spike fails its acceptance criteria, fall back to route 2. Route 3 stays post-PWA-validation. Any route 1 ship also requires a `D54` reconsideration (no-background-audio decision) since even ambient audio is *some* audio. Captured from founder probe 2026-04-21; cross-reference `D42` (run-mode baseline), `D54` (no background audio), `D57` (iOS Safari PWA constraints), `D122` (Phase F audio carve-out).
