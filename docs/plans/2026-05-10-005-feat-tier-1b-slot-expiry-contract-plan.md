...
- **Calendar-time guard fires as a warning, not a hard failure, until 2026-07-20.** Hard failure post-expiry on any remaining `reserved` slot. Before expiry, the validator confirms structure only — `last_checked` staleness is not enforced. (Historical: this plan originally named the weekly Monday adversarial-memo ritual as the staleness mechanism; that ritual was **retired 2026-05-27 per `D148`** following M001 executive closure. Staleness checks now happen at trigger-fire / kill-decision time or at the 2026-07-20 hard expiry, both of which are independent of the retired ritual cadence.)
