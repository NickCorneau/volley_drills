...
- **What this pass does NOT do.** Larger walkthrough-surfaced items — Persist Net/Wall, Auto-fill training recency, ~~Visual block-end countdown cue~~ (landed 2026-04-26 by founder direct approval, see §Deferred row 8), effort/tag state anomaly reproduction, Skip-review confirmation modal, first-session-only verdict string on Complete — are **explicitly deferred** to founder-session-trigger-gated Tier 1b work. Listed in §Deferred below.
...
- **`Done` / `Finish later` hierarchy**: `Done` is the full-width primary action; `Finish later` is a lower-emphasis link-style escape hatch. The equal-primary proposal was tried, then reversed by 2026-04-24 founder design feedback because the two actions became unclear.
...
4. `Done` renders as the primary CTA and `Finish later` renders below it as a lower-emphasis link-style action.
...
| Visual block-end countdown cue on Run (progress bar + "0:47 left" chip) | 8 | ~~New visual primitive on the run screen; behavior-adjacent.~~ **Landed 2026-04-26** in `app/src/components/BlockTimer.tsx` (bar `h-2` → `h-3`; trailing `"X:XX left"` chip; big-digit final-3-sec accent flip preserved as the single visual countdown tick). Coverage: `app/src/components/__tests__/BlockTimer.progress-chip.test.tsx`. Founder approved the build directly (recommended-ship #8) without waiting for the listed founder-session trigger; treat the trigger column as moot, not as a violation of the cap discipline (item is editorial-class once decomposed: a single component edit + tests, no behavior or persistence change, no metadata schema change). | ~~Founder logs ≥1 session where the block-end was missed despite the audio tick.~~ **Discharged.** |
...
- `app/src/screens/ReviewScreen.tsx` — QuickTagChips card deleted; divider added; Good-passes visibility gate updated; Submit → Done; Finish later restored to link-style lower-emphasis action; 2-hour countdown removed.
